...
더욱 확률 높고 트래픽을 몰고 올 수 있는 키워드가 틈새키워드". 지금 탭은 데이터랩 수요 + 문서수 3,000↓ 만 보고
틈새라 불렀다 — 검색량 게이트도, 진짜 SERP 자리 실측도 없다(오픈API 블로그 메타 ≠ 노출 순위).

## 판정 재정의 (`issue-niche-verdict.ts`)

- 틈새 = **트래픽** (검색광고 실측 ≥ 300, 또는 ≥ 100 이면서 데이터랩 수요 상승; 추정치·10 미만 제외)
  AND **수요** (데이터랩 최근 7일 실측, 죽지 않음, 도배 아님, 문서수 3,000↓ 실측)
  AND **자리** (네이버 블로그탭 상위 10 제목 정면 대응 0건 = WINNABLE, Bright Data 실측)
- 자리 미실측(회차 상한 12건 밖) 이면 `pending` — 틈새로 싣지 않고 다음 회차에 잰다.
- CONTESTED/LOCKED 는 틈새 탈락(원장에만 남음).
- 선점 후보 = 기존(문서수 300↓·수요 미검출) + 수요는 잡혔으나 검색량이 아직 없는 문서수 300↓ 행("수요 잡힘·검색량 미확인" 배지).
- 재측정·자리 실측 뒤 판정을 **다시 내린다** — 검색량이 생기면 자동 승격.

## Bright Data 예산 (사장님 승인)

- 회차당 12건(검색량 큰 순), 하루 3회 = 36건, 월 ≤ 1,116건. 기능 'issue' 별도 상한 1,100.
- 이미 잰 키워드(48h 안)는 재사용. 원장은 사이트 레포 `data/brightdata-quota-issue.json` 으로 회차마다 커밋(황금 원장과 분리, 합쳐도 $10 한도 안).

## Phase

1. 판정 재정의 + 테스트(RED→GREEN) — `issue-niche-verdict.ts`, 헌터 판정 호출부
2. `src/utils/issue-slot-measure.ts`(순수: 대상 고르기·입히기·재판정) + `scripts/serp-slot-issue-board.js`(BD 호출) + 워크플로 배선(회차 뒤·보강 앞, 재측정 뒤 재판정) + 워크플로 계약 테스트
3. 발행기 `serp` 필드·`pending` 제외·무료 표본은 확정 틈새만 / 사이트 `issueFlow.ts`·`IssueNicheTab.tsx`·`PreemptionCard.tsx` 자리 배지
4. 테스트·tsc → 커밋(분리 커밋, --no-verify 금지) → 푸시 → `gh workflow run` → 로그로 건수·비용 확인 → 사이트 빌드·배포 → 라이브 Playwright 확인 → 최종 보고

## 이전 계획

# 실행 계획: 실검 틈새 — 이슈 추론 계층 (왜 뜨나 → 몰린 곳 → 다음 물결 → 선점)

승인 2026-09-03 ("응 진행하고 그걸 볼수있는게 실시간 검색어 브릿지자나 그게 너무빈약해지금보면").
시작 지점: `f9367afa`(무료 3건). 보이는 자리 = 사이트 홈 실시간 검색어 **기사 브리프**(SourceBriefModal) + 실검 틈새 탭.

### 사장님 요구 (그대로)

"프로필을 나오게할게아니라 지금 왜뜨는지 어느키워드에 몰려있으니까 다음 사람들이 궁금해하는키워드가 뭔지
어떤키워드를 선점해놔야 트래픽이 몰릴지 분석해서 올려놔야되는거아니니?? 그 추론분석부분이 빠져있자나"

### 구조 (황금키워드보드 방식 그대로 — 사장님 CI 3회/일, 정적 JSON, 방문자는 보기만)

1. `issue-context.ts` (신규) — 이슈별 재료 실측: 뉴스 헤드라인 5(오픈API 뉴스) · 자동완성(PC+모바일, 경량) · 검색광고 연관어(월 검색량)
2. `issue-next-wave.ts` (신규) — 구독 에이전트 **1콜**(후보 생성 콜과 합침): 재료 → {why(헤드라인 근거), stage, cands, nextWave[{keyword, reason}]}. 환각 가드: why 의 숫자는 헤드라인에 있어야 하고 헤드라인 어절 2개 이상 공유, 아니면 why 버림(헤드라인만 노출). nextWave 는 3어절·상업접미사 금지·측정 통과분만 실린다
3. `issue-niche-hunter.ts` — 후보 = nextWave + 자동완성 + 파생 + 연관어(origin 표시), 실측·게이트는 그대로. 새 `huntIssueNicheBoard()` → {rows, issues}
4. `issue-briefing.ts` (신규, 순수) — 이슈별 ①왜 뜨나 ②지금 몰린 곳(실측 수요·문서수·자리) ③다음 물결(실측 붙여) ④선점 추천(틈새·선점 판정 통과분)
5. 발행기 `issues[]` + 행 `origin/reason` (추정 숫자 없음, 48h 이월). CI maxCandidates 80→160 (데이터랩 ≈50/회 = 15%/일)
6. 사이트: `IssueFlowCard` 공용 → 실검 틈새 탭 상단 이슈 흐름 + 홈 실시간 브리프 모달(키워드 매칭). 비로그인 ①② 공개, ③④ 잠금. 무료 3행 규칙 유지

### 추가 지시 2026-09-03 — 황금키워드 카드와 **똑같이**

"황금키워드랑 똑같이 이런식으로 버튼이랑 연관키워드 등등 그래프도 똑같이 전부 잘나와야되는거아닌가" (황금 카드 스크린샷 첨부).

- 회차 = 발굴 → **황금 보강기 그대로**(`enrich-board.js`: 지식인 수·최신 질문, 연관 실측 풀+AI 검증, 서브키워드, SEO/홈판 제목, 30일 추세, 수익 판정) → 발행.
  `issue-niche-board-shape.ts` 가 보강 대상(틈새·선점 행)을 황금 행 모양으로 고른다(topic·intentLabel·adsenseFit·issueHeadlines). 보강 프롬프트는 뉴스 헤드라인을 실측 근거로 받는다.
- 공개 행 = 황금 행과 같은 필드(evidence·whySearch·titles·subKeywords·keywordPool·trend·kinCount·kinTop·monetize·adsense*) + 이슈 필드(origin·originReason·정면 글). `issues[]` 브리핑(왜·헤드라인·몰린 말·다음 물결·보드 합류 표시).
- 사이트: 황금 탭 카드 본체를 `PreemptionCard.tsx` 로 떼어 두 탭이 같이 쓴다(지표 줄만 변형: 광고수·빈자리 → 정면 글·최근 정면). 4버튼(어떻게 쓸까·네이버 검색결과·LEWORD 분석·마인드맵)·스파크라인·풀 칩·수익 칸 전부 동일.
- 빠진 것(사장님 결정 필요): SERP 실측(광고수·빈자리·상위 제목)은 Bright Data 호출이라 뺐다 — 8월 황금 6,347콜 > 무료 5,000, 이슈 보드가 붙으면 월 +2,700.

### 진행

- [x] Phase 0/1 재료 수집 + 테스트 (issue-context 8건)
- [x] Phase 2 에이전트 추론 + 가드 + 테스트 (issue-next-wave 11건)
- [x] Phase 3 헌터 배선 · 발행 스키마 · 황금 보강 연결 · 스크립트/워크플로 (68건 통과, tsc 0, build 0)
- [x] 로컬 검증 회차(issueLimit 6·maxCandidates 30·maxAi 4): 이슈 14 → 후보 30 → 틈새 4·선점 3, "왜" 검증 14/14, 다음 물결 69, 보강 4행(제목·서브·풀·지식인·추세)
- [ ] Phase 4 사이트(PreemptionCard 추출 → 틈새 탭 + 브리프 모달) · 로컬 렌더 검증
- [ ] Phase 5 CI 회차 → 검수 → 푸시(앱·사이트) → 데스크톱 릴리즈 → 보고
