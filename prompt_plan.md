...
- 2026-07-15 사람 검수 게이트 운영 경로 구현: API는 `/data/live-golden-human-review.json`을 쓰지 않고 읽기만 하며, exact `boardUpdatedAt`·versioned Verified fingerprint·전수 reviewed·서버 계산 정밀도·malformed/의미중복/플랫폼잔재/문장잔재 0건이 모두 맞을 때만 `superiorityGate=pass`가 된다. 파일 누락·변조·보드 변경은 reason code와 함께 `pending-human-review`, 현재 보드에 귀속된 실제 검수 미달은 `fail`로 닫힌다. 사람 전수 검수 전에는 이 파일을 생성하지 않는다.
