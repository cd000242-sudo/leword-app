...
> 작성일: 2026-07-11
> 상태: **전 페이즈 사용자 승인 완료 / Phase 0 완료 / Phase 1D 자동 공급·AI 검수 통과 / 사람 전체 검수 대기 / Phase 2 진입 게이트 미통과**
> 복잡도: 높음
...
- 2026-07-14 `1364a599` 워커 전용 배포 검증: API digest는 유지하고 워커만 교체했다. 8회 bounded catch-up은 실패 0, SearchAd 83회 사용, 금융→생활주거→반려동물→쇼핑뷰티→자동차 결손 순환을 확인했지만 신규 Verified는 0이었다. 자연어 완성형의 실제 값도 `자동차 검사 예약` 49,500/73,507(비율 0.67), `강아지 미용 비용` 300/169,113(0.002) 등 합격선에 미달했다. 다음 개선은 기준 완화가 아니라 결손 카테고리당 최대 4개의 짧은 SearchAd 앵커로 실제 검색량이 존재하는 연관 표현을 먼저 수집하고, 남은 공유 측정 예산 안에서 문서수를 붙이는 방식이다. 뉴스·프로필·플랫폼 잔재는 문서수 호출 전에 차단한다.
- 2026-07-14 `1c12f275` 워커 전용 배포 검증: API digest를 유지한 채 새 워커로 8회 catch-up을 완료했으며 실패 0, SearchAd 84회 사용, Verified 8·활성 코어 2·최대 점유율 75%·완전성 88.89%·untrusted 1로 공급 증가는 0이었다. 운영 원문 진단에서 SearchAd는 `자동차검사비용`, `엔진오일교환비용`, `타이어위치교환비용` 같은 실제 검색어를 반환하지만 `car`와 `car_maintain`의 동일 코어 정책 불일치 및 공백 없는 7~8자 `비용` 검색어의 broad-head 오판 때문에 문서수 측정 전에 탈락하는 병목을 확인했다. 수정은 동일 코어 정책의 앵커 추천만 호환하고, 실측 가능한 자연어 추천의 중복 고수익 사전차단을 제거하며, 7자 이상 구체적 비용·예약·계산 복합어를 broad head에서 제외한다. 집중 재현에서 `엔진오일교환비용`은 검색량 3,000·문서수 120·비율 25·AI 100/publish·SSS로 최종 보드에 진입했고 `자동차보험 공식입장`은 계속 차단됐다. 운영 게이트 기준과 SearchAd ceiling은 변경하지 않는다.
- 2026-07-14 `48b64f04` 워커 전용 1차 canary: 배포 후 7회 실행·실패 0, SearchAd 4,144→4,220으로 앵커 호출은 정상이었고 `home_life`에서 320개 중 `에어컨이전설치비용`, `원룸입주청소비용`, `보일러교체비용` 등 12개를 자연 검색어로 선별했다. 다만 운영에서는 `anchors` 이후 `measurement`가 발생하지 않았고 Verified 8·활성 코어 2·최대 점유율 75%·완전성 88.89%·untrusted 1이 그대로였다. 원인은 해당 카테고리의 기존 합성/카탈로그 측정 후보가 0개일 때, 이미 SearchAd에서 실측·선별된 제안 행이 있어도 측정 직전 조기 반환하는 것이다. 제안 행 자체를 후보 집합에 포함해 이 반환 경로를 제거했고, 게이트·쿼터·ceiling은 변경하지 않았다.
- 2026-07-14 `643f06ed` 워커 전용 2차 canary: `home_life` 12개가 전부 문서수 측정으로 이어져 조기 반환 수정은 운영에서 입증됐다. 그러나 원문 문서수는 `에어컨이전설치비용` 268,145, `원룸입주청소비용` 122,701, `브레이크오일교체비용` 113,608, `엔진오일교체비용` 281,929로 구성 단어까지 넓게 포함했다. 동일 키워드의 Naver OpenAPI 정확구문 경쟁 문서수는 각각 14,588, 3,027, 677, 5,242로 확인됐다. SearchAd에서 실검색량이 입증된 curated 후보에만 `exact-phrase` 경쟁 문서수를 적용하고, 기존 broad persistent cache는 우회해 두 측정 의미가 섞이지 않게 한다. 해당 근거는 `naver-openapi-exact-phrase`로 보존하며 실측/신뢰/품질 게이트는 그대로 유지한다.
- 2026-07-15 Phase 1D 운영 자동 공급 게이트 통과: 엄격한 재로딩 후 자연 워커 승격 3회에서 Verified `41→46→54→60`, 최근 7일 활성 코어 `11`, 최대 코어 점유율 `15%`, 측정 완전성 `100%`, stale/untrusted/unknown `0`을 확인했다. SearchAd quota reset·ceiling 상향·수동 `/run` 없이 달성했으며 최종 health failureReasons는 빈 배열이다.
- 2026-07-15 전체 60개 AI 블라인드 재검수: 자연 검색어 `60/60`, 의도 일치 `60/60`, 확정 의미 중복 `0`, 플랫폼·문장 잔재 `0`, malformed `0`, 정밀도 `100%`. 다만 사람 전체 검수는 아직 완료되지 않았으므로 `superiorityGate=pending-human-review`와 Phase 2 진입 보류를 유지한다.
- 2026-07-15 `1023bed2` 웹 표시 불일치 수정: 인증 snapshot은 60개지만 기존 GitHub Pages 정적 iframe이 브라우저 휴리스틱으로 정상 publish 행 12개를 재필터링해 `48/48`만 표시했다. 인증 Pro 보드는 서버 순서·등급을 그대로 렌더링하고 공개/브라우저 폴백만 기존 보호 필터를 유지하도록 수정했으며 전체 sanity를 통과했다. API image `sha256:416de081...`만 재배포했고 워커는 기존 image/start time을 유지했다. API 생성본에는 수정이 반영됐지만 `leaderspro.kr/leword-pro-web.html`은 별도 GitHub Pages 정적 배포 대상이므로 API-only 승인 범위에서는 운영 웹 사본이 아직 갱신되지 않았다.
- 2026-07-15 사람 검수 게이트 운영 경로 구현: API는 `/data/live-golden-human-review.json`을 쓰지 않고 읽기만 하며, versioned Verified 의미 fingerprint(키워드·카테고리·의도)·검수 스냅샷 시각·전수 reviewed·서버 계산 정밀도·malformed/의미중복/플랫폼잔재/문장잔재 0건이 모두 맞을 때만 `superiorityGate=pass`가 된다. 키워드·카테고리·의도 또는 Verified 구성 변경은 즉시 무효화하지만, 동일 의미 보드의 검색량 재측정·내부 행 ID·행/보드 갱신 시각 변경만으로 실제 사람 검수가 12분마다 무효화되지는 않는다. 파일 누락·변조·의미 보드 변경은 reason code와 함께 `pending-human-review`, 현재 보드에 귀속된 실제 검수 미달은 `fail`로 닫힌다. 사람 전수 검수 전에는 이 파일을 생성하지 않는다.
- 2026-07-18 Phase 2 진입 계약 보강: 검수 cohort를 의미 family당 1개로 제한하고, 모든 사람 판정에 `hiddenKnown`을 필수화해 뻔한 키워드가 1건이라도 있으면 인증서를 발급하지 않는다. 검수 실패는 첫 실패 시점에 불변 audit tombstone으로 먼저 보존해 과거 cohort 파일 복원으로 재통과할 수 없게 했으며, 엄격한 관리자 웹 세션은 독립된 32바이트 이상 서명 secret과 고정 HTTPS 인증 공급자만 사용한다. 고정 hidden-known 벤치마크는 정밀도·재현율 100%, malformed·플랫폼/문장 잔재·의미/패밀리 중복·obvious head 0건을 통과했지만 실제 운영 cohort의 사람 전수 검수는 아직 수행하지 않았다.
- 2026-07-18 분석기/황금키워드 문서수 SSoT 보강: 문서수는 네이버 블로그 OpenAPI broad 쿼리 키를 그대로 보존하고 검색량은 SearchAd compact 키에만 결합한다. 공백 별칭을 요청 키워드로 바꿀 때 별칭 broad 문서수를 재귀속하지 않으며, 검색량·문서수 각각 신뢰 가능한 더 최신 측정만 채택한다. 서로 다른 행/시각에서 결합되면 과거 score·grade·AI 판정을 재사용하지 않고 새 지표로 다시 계산한다. 화면에는 `문서수(네이버 블로그·확장검색)`과 측정 출처·시각을 유지한다.
- 2026-07-18 문서수 최종 실패 폐쇄 보강: 2026-07-14에 사용했던 exact-phrase 경쟁 문서수는 현재 Verified의 신뢰 가능한 broad 문서수 근거로 더 이상 인정하지 않는다. 따옴표가 포함된 레거시 캐시는 폐기하고, 분석기·발굴기·프리미엄 헌터·라이프사이클·저장 키워드 갱신·브라우저 폴백이 모두 정규화된 동일 broad Blog OpenAPI client를 사용한다. API 실패·쿼터·잘못된 응답은 `null/미측정`으로 닫고 실제 `total=0`만 0으로 보존하며, 개발 모드에서 우선 해석되는 tracked JavaScript mirror의 임시 100 추정도 제거했다.
- 2026-07-18 문서수 질의 결속 보강: 문서수 실측에는 정규화된 broad 질의 전체를 `documentCountQueryKey`로 함께 저장하고, 현재 표시 키워드에서 계산한 키와 완전히 같을 때만 신뢰한다. 따라서 공백 유무가 다른 별칭이나 과거 캐시의 문서수를 재사용하지 않으며, 키가 없거나 불일치하거나 재측정에 실패한 값은 문서수·비율·등급·점수·과거 AI 파생판정을 함께 `null/미측정`으로 닫고 새 canonical broad 실측이 성공해야만 Verified로 복귀한다. 복구 대기 행은 공개 보드가 아닌 최근·제한 수량의 비공개 재측정 재고로 저장해 쿼터 대기나 워커 재시작에도 소실되지 않게 한다. SearchAd 검색량의 compact 결속은 이 문서수 질의 결속과 독립적으로 유지한다.
- 2026-07-18 숨은 수요 공급 경로 보강: 홈 탭의 `admin-image-ocr-reviewed` 직접 검수 시드는 모든 출처·최신성·실수요·문서수·품질·중복 게이트를 통과할 때만 코어 후보로 연결하고, 자동완성·비코어·뻔한 연도+정책+단일 행동형은 계속 Surge/차단으로 유지한다. 이는 `부방장`이라는 문자 자체를 찾는 규칙이 아니라 사람들이 알지만 경쟁 발굴 도구가 놓치기 쉬운 구체적 의도형을 찾는 규칙이다. v2 전수 검수에서 `hiddenKnown=false`가 1건이라도 있으면 인증서뿐 아니라 `/health`의 `superiorityGate`와 검수 reason도 실패로 닫는다.
- 2026-07-18 운영 배포 안전성 보강: API 전용 `/data`, 황금키워드 `/golden`, 사람 검수 `/review`, 홈 검수 시드 `/briefing`, SearchAd 계정 `/searchad`, 공유 쿼터 `/quota`로 권한을 분리했다. `/briefing`은 API만 쓰고 워커는 읽기 전용으로 사용하며 기존 `/data/home-keyword-briefing.json`은 digest 표식과 원본 보존을 갖춘 전진·롤백 마이그레이션으로 이동한다. SearchAd는 HTTP 전에 ceiling 확인과 사용량 예약을 하나의 교차 프로세스 잠금 트랜잭션으로 수행하고 실패·중단도 보수적으로 계상한다. 네이버 OpenAPI 쿨다운도 잠금 안에서 최신 파일을 다시 읽어 키별 max/union으로 원자 저장한다. 전진·롤백 마이그레이션은 사용량을 감소시키지 않고 기존 원본을 보존하며, digest 검증은 compose 교체·서비스 정지보다 먼저 수행한다.
- 2026-07-18 코드 검증은 전체 레이더, API/검수/문서수/쿼터/배포 집중 회귀, 루트 빌드와 API typecheck를 통과했다. 그러나 현재 변경은 아직 운영에 배포하지 않았고 운영 토큰은 노출 이력 때문에 먼저 회전해야 한다. 정확 digest 배포 후 운영 60/10/18%/100%/불신 0을 다시 실측하고 실제 전체 cohort 사람 블라인드 검수에서 정밀도 90% 이상·malformed/중복/플랫폼/문장/obvious 0건을 확인해 인증서를 발급하기 전에는 Phase 2 진입으로 기록하지 않는다.
- 2026-07-18 SearchAd 최소 구간 정확성 보강: API의 문자열 `<10`은 숫자 0이 아니라 미확정 범위로 보존한다. PC/모바일 중 하나라도 `<10`이면 총검색량·비율·등급 계산과 정확값 캐시를 막고 `partial/C/미측정`으로 닫으며, 실제 숫자 `0`만 정확 실측으로 인정한다. 이 정책은 PC 엔진뿐 아니라 PRO 트래픽, Traffic Explosion, lifecycle tracker, 연관키워드 fallback까지 동일하게 적용한다.
- 2026-07-18 외부 AI 보조 비용·신뢰 경계 보강: 기본값은 off라 호출과 비용이 0이다. 사용자가 자기 API 키의 소유 범위와 단일 공급자를 명시적으로 선택한 경우에만 상위 8행을 최대 출력 2,048토큰으로 한 번 호출하며, 실패 시 다른 공급자로 자동 재시도하지 않는다. 서버 키는 `LEWORD_ALLOW_SERVER_EXTERNAL_AI=1`의 별도 운영 승인 없이는 사용할 수 없다. AI는 검색량·문서수·비율·등급 같은 구조화 실측을 바꾸지 못하고, 생성 연관어는 미실측으로 표시한다. Codex/Claude Code 선택은 실제 추론 실행이 아니라 readiness/status-only로 정직하게 표시한다.
- 2026-07-18 실측 복구 표기 보강: 브라우저 fallback이나 서버 단절 결과를 자동 복구된 실측으로 표시하지 않는다. 문서수 또는 검색량이 없으면 `미측정 · 서버 연결 후 다시 실행`으로 안내하고, 재연결 뒤 사용자가 다시 실행해 canonical API 실측이 성공하기 전에는 문서수·비율·등급·Verified를 복구하지 않는다.
...
**신뢰 원칙(2026-07-20 추가, 감량과 동일 방향):** 황금키워드↔키워드분석↔타사 수치 불일치 우려 → 목표는 타사 일치가 아니라 ①내부 무모순(측정 SSoT — 병렬 측정 경로 삭제, Codex canonical 바인딩 완주, 분석 실측 시 보드 행 수렴 갱신) ②측정 투명성(수치 옆 실측시각·원천·30일롤링·문서수 쿼리방식 표기) ③"왜 타사와 다른가" 설명 툴팁/페이지. 관찰 기간 불일치 상당수는 광역쿼리 마이그레이션 과도기 산물.
