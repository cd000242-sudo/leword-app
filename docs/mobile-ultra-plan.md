...
- `.github/workflows/mobile-release.yml` publishes the production API worker image to GHCR on the `api-image` and `full-release` paths, then uploads the immutable SHA tag as `mobile-api-image-reference`.
- `apps/api/docker-compose.production.yml` runs the GHCR image on a production host with `apps/api/.env.production`, `/health` healthcheck, port `34983`, and a persistent `leword-mobile-cache` volume for `LEWORD_MOBILE_CACHE_FILE=/data/mobile-cache.json`.
...
- `/health.runtime` exposes the same API worker readiness report without leaking secrets.
