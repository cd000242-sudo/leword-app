...
async function proposeSubKeywords(mainKeyword, groundingExamples, relatedCandidates, serpEvidence) {
  ...
  const prompt = [
    ...
    `  공식: 메인키워드("${mainKeyword}") 를 앞부분에 한 번만 + 위 실측 서브키워드 하나`,
    '  + 궁금증 후킹. 키워드는 문장 속에 자연스럽게 녹이고, 억지 반복은 하지 않는다.',
    ...
    '  ③ 답·결론은 제목에서 공개하지 않는다 — "이러니까"의 정체는 본문에만. 궁금증은',
    '     "대부분 놓치는", "의외로 다른", "많이들 착각하는", "여기서 갈린다" 같은 형태로.',
    '  ④ 감정은 직접 말하지 않고 상황으로 보여준다("놀랐다" X → "영수증을 다시 봤다" O).',
    '     사람의 흔적(삽질·후기·실제 오류 문구)을 쓰되 없는 수치·없는 경험은 금지.',
    '  ⑤ 숫자(시간·기간·횟수·금액·순위·비교)가 실측 사실에 있으면 하나 이상 넣는다 — 없는 숫자 금지.',
    '  ⑥ 신뢰: "무조건"·"100%"·"평생"·"총정리"·"완벽 가이드"·"한눈에" 금지. 느낌표·감탄사 남발 금지.',
    '     대신 "생각보다"·"의외로"·"많이들"·"자주" 같은 현실적인 표현으로 궁금증·감정을 끈다.',
    ...
    '  본문이 못 지킬 약속·과장·허위는 전부 실패다.',
    '  짓고 나서 자문하라: "AI 가 지었다고 눈치챌 구석이 하나라도 있나? 누구나 다 하는 제목인가?',
    '  왜 클릭하는지 읽는 사람이 알 수 있나? 과장·낚시 없이 믿을 만한가?"',
    '  하나라도 걸리면 버리고 다시.',
...
async function retryHomeTitle(mainKeyword, subKeywords, previous, fails) {
  ...
  const prompt = [
    ...
    '조건(전부 지켜야 한다):',
    `  · "${mainKeyword}" 가 제목 앞부분에 한 번만 들어간다`,
    '  · 35~45자',
    '  · "키워드, 후킹문" 쉼표 이분법 금지 — 그 틀 자체가 AI 서명이다',
    '  · "핵심 정리"·"총정리"·"한눈에"·"알아보"·"확인할 점"·"무조건"·"100%"·"평생"·"완벽 가이드" 금지',
    '  · 말하듯 쓴다 — ~네요/~어요/~더라고요. 문어체 완결문·느낌표 남발 금지',
    '  · 답·결론은 제목에서 공개하지 않는다(궁금증만 남긴다)',
    '  · 감정은 상황으로 보여준다("놀랐다" X → "영수증을 다시 봤다" O)',
    '  · 없는 수치·없는 경험을 지어내지 않는다. 과장·낚시 금지, "생각보다"·"의외로" 같은 현실 표현',
    '',
