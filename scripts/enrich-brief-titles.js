...
function collectRows(signals) {
  ...
  for (const lane of signals.lanes || []) {
    for (const item of lane.items || []) {
      ...
      rows.push({ keyword, lane: lane.label || lane.id || '', facts });
    }
  }
...
function buildPrompt(batch) {
  return [
    ...
    '- home: 목록에서 눌러보고 싶게 만드는 제목. 38자 이내. 과장·낚시 금지,',
    '  사실 안에서 가장 궁금한 지점을 앞세운다.',
    '- 두 제목 모두 검색어의 첫 단어를 반드시 포함한다.',
    '- "최신 이슈와 핵심 내용 정리", "관련 확인할 점" 같은 상투구 금지.',
    '',
...
async function titlesForBatch(batch) {
  ...
  for (const entry of parsed) {
    ...
    const home = usableTitle(entry.home, keyword);
