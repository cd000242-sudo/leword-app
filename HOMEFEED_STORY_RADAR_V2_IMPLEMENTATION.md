...
**실측으로 찾아 고친 결함 2건**
1. 네이버 뉴스 검색이 문장형 검색어에 느슨하게 걸어 준 **다른 이슈 기사**가 표본에 섞여 각도 · 재미 근거를 오염시켰다(예: '엄지성 해트트릭' 이슈에 '손흥민 기념식' 기사). 검색어 어절이 제목에 실제로 겹칠 때만 표본으로 쓰도록 필터를 넣고(`isRelevantSample`) 버린 기사 수를 회차 로그에 남긴다.
2. 기준어가 **너무 넓은 앞어절 하나**로 잡혔다 — '서울 시내버스 협상 타결' → '서울', 그래서 첫 카드 문구가 "서울 철회"가 됐다. 지역 · 나라 이름이면 두 어절까지 넓힌다(`anchorOf` · '서울 시내버스' · '조지아 한국인', 사람 이름은 그대로).
