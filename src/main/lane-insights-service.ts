/**
 * 레인 인사이트 서비스 — 키워드 하나에 "문제해결 서브 3 + 제목 2종"을 만든다.
 *
 * 소비처가 둘이라 IPC 핸들러에서 분리했다:
 *   ① 데스크톱 UI (forge-lane-insights IPC)
 *   ② 웹 브리지 (사이트가 사용자 PC 의 클로드코드를 쓰는 통로 — web-bridge)
 *
 * 추론 체인(사장님 지시: "에이전트가 두뇌 역할"):
 *   ① 규칙+실측: 자동완성 확장(무료) → 검색광고 검색량 → 문제형 프레임 선별
 *   ② 서브가 3개 미만이면 구독 CLI(Claude Code→Codex→Gemini 순, 감지된 것)가
 *      파생을 제안하고, 제안은 반드시 실존 결재(검색량>0 또는 자동완성 프로브)를
 *      통과해야 합류한다 — AI 가 제안하고 실측이 결재한다.
...
export interface LaneInsightsResult {
  success: true;
  keyword: string;
  subs: SubKeyword[];
  titles: ForgedTitles;
  expansionCount: number;
  volumesMeasured: number;
...
export async function forgeLaneInsights(rawKeyword: string): Promise<LaneInsightsResult> {
  ...
  const titles = forgeTitles({
    ...
    // 온디맨드 경로에는 SERP 실측이 없다 — 빈 배열이면 프레임 분석이
