 ...
 */
async function getBokjiroRealtimeKeywordsWithPuppeteer(limit = 10) {
    let browser = null;
    try {
        ...
        const keywords = await page.evaluate((maxLimit) => {
                ...
                rankMenus.forEach((rankMenu, menuIndex) => {
                    ...
                    keywordElements.forEach((keywordEl, kwIndex) => {
                        ...
                        if (keywordText &&
                            ...
                            // 중복 체크
                            const isDuplicate = result.some(item => item.keyword.toLowerCase() === keywordText.toLowerCase());
                            if (!isDuplicate) {
                                result.push({
                                    keyword: keywordText,
            ...
            if (result.length < maxLimit) {
                ...
                for (const selector of alternativeSelectors) {
                    if (tempKeywords.length >= maxLimit)
                        break;
        ...
        if (browser) {
            try {
                ...
                console.warn('[BOKJIRO-REALTIME] 브라우저 종료 오류:', e);
            }
        }
    }
}
