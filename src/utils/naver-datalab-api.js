 ...
 */
async function getNaverTrendKeywords(config, options = {}) {
    const { keywords = [], startDate = getDateDaysAgo(30), // 기본 30일 전
    endDate = getDateToday(), timeUnit = 'date', device = 'pc', // 기본값이지만 실제로는 모바일+PC 합산 데이터가 더 정확
    ages = [], gender } = options;
    if (!config.clientId || !config.clientSecret) {
        throw new Error('네이버 API 인증 정보가 필요합니다 (Client ID, Client Secret)');
    }
    // 캐시 키 생성
    const cacheKey = api_cache_1.apiCache.generateKey('naver-datalab-trend', {
    ...
    // 캐시된 API 호출 (10분 TTL)
    return (0, api_cache_1.cachedApiCall)(cacheKey, async () => {
        try {
...
기능은 계속 작동하지만, 데이터랩 기반 트렌드 분석은 건너뜁니다.`;
            ...
            const response = await error_handler_1.ErrorHandler.withRetry(async () => {
                return await error_handler_1.ErrorHandler.withTimeout(async () => fetch(apiUrl, {
                    method: 'POST',
                    headers: headers,
            ...
            if (!response.ok) {
                const errorText = await error_handler_1.ErrorHandler.safeString(await response.text().catch(() => ''), '');
...
3. 문제가 지속되면 네이버 고객센터에 문의`;
                ...
                const friendlyMessage = error_handler_1.ErrorHandler.getFriendlyMessage({ status: response.status, message: errorText }, '네이버 데이터랩 API');
                throw new Error(friendlyMessage);
            }
            const responseText = await response.text().catch(() => '{}');
            const data = error_handler_1.ErrorHandler.safeJsonParse(responseText, { results: [] });
            ...
            if (error instanceof TypeError && error.message.includes('fetch')) {
                ...
                const isRealNetworkError = errorMsg.includes('failed to fetch') ||
 ...
 */
async function getNaverRankingKeywords(config) {
    if (!config.clientId || !config.clientSecret) {
 ...
 */
async function getBlogSearchFallback(config, keyword) {
    try {
        const apiUrl = 'https://openapi.naver.com/v1/search/blog.json';
        const params = new URLSearchParams({
            query: keyword,
        ...
        // 보수적으로 문서수 * 0.3으로 추정
        const estimatedSearchVolume = Math.floor(total * 0.3);
        // PC와 모바일 비율은 일반적으로 2:8 (모바일이 더 많음)
        const pcVolume = Math.floor(estimatedSearchVolume * 0.2);
        const mobileVolume = Math.floor(estimatedSearchVolume * 0.8);
        console.log(`[NAVER-VOLUME] 블로그 검색 API 폴백 "${keyword}": 문서수=${total}, 추정 검색량=PC ${pcVolume}, 모바일 ${mobileVolume}`);
        return {
            keyword: keyword,
 ...
 */
async function getNaverKeywordSearchVolumeSeparate(config, keywords) {
    if (!config.clientId || !config.clientSecret) {
        throw new Error('네이버 API 인증 정보가 필요합니다');
    }
    const results = [];
    for (const keyword of keywords) {
        try {
            ...
            // 검색량 조회용: 띄어쓰기 제거
            const processedKeyword = originalKeyword.replace(/\s+/g, '');
            let pcVolume = 0;
            let mobileVolume = 0;
            let documentCount = 0;
            // 병렬 처리: 검색량(검색광고 API) + 문서수(블로그 검색 API) 동시 조회
            const [searchVolumeResult, documentCountResult] = await Promise.allSettled([
                // 1. 검색량: 검색광고 API (띄어쓰기 제거 버전)
                (async () => {
                    const { getNaverSearchAdKeywordVolume } = await Promise.resolve().then(() => __importStar(require('./naver-searchad-api')));
                    const envManager = (await Promise.resolve().then(() => __importStar(require('./environment-manager')))).EnvironmentManager.getInstance();
                    ...
                    let customerId = envConfig.naverSearchAdCustomerId;
                    if (!customerId || customerId.trim() === '') {
                        ...
                        if (parts.length > 1 && firstPart && firstPart.trim() !== '') {
                            ...
                            console.warn(`[NAVER-VOLUME] ⚠️ customerId가 명시적으로 설정되지 않아 accessLicense에서 추출: "${customerId}". 정확한 customerId(예: 3992868)를 NAVER_SEARCH_AD_CUSTOMER_ID 환경 변수에 설정하세요.`);
                        }
                    }
                    const finalCustomerId = (customerId && customerId.trim() !== '') ? customerId.trim() : undefined;
                    ...
                    // 띄어쓰기 제거된 키워드로 검색량 조회
                    const timeoutPromise = new Promise((_, reject) => setTimeout(() => reject(new Error('검색광고 API 타임아웃 (60초 초과)')), 60000));
                    const searchAdResults = await Promise.race([
                        getNaverSearchAdKeywordVolume(searchAdConfig, [processedKeyword]),
                        timeoutPromise
                    ]);
                    if (searchAdResults && searchAdResults.length > 0 && searchAdResults[0]) {
                        const result = searchAdResults[0];
                        const pcVol = result.pcSearchVolume || 0;
                        const mobileVol = result.mobileSearchVolume || 0;
                        if (pcVol > 0 || mobileVol > 0) {
                            console.log(`[NAVER-VOLUME] ✅ 검색광고 API "${processedKeyword}" (원본: "${originalKeyword}"): PC=${pcVol}, 모바일=${mobileVol}`);
                            return { pcVolume: pcVol, mobileVolume: mobileVol };
                        }
                    }
                    return { pcVolume: 0, mobileVolume: 0 };
                })(),
            ...
            if (searchVolumeResult.status === 'fulfilled') {
                ...
                const errorMessage = searchVolumeResult.reason?.message || String(searchVolumeResult.reason || '');
                console.warn(`[NAVER-VOLUME] ⚠️ "${originalKeyword}" 검색량 조회 실패: ${errorMessage}`);
            }
            // 문서수 결과 처리
            if (documentCountResult.status === 'fulfilled') {
                ...
                console.warn(`[NAVER-VOLUME] ⚠️ "${originalKeyword}" 문서수 조회 실패: ${documentCountResult.reason?.message || String(documentCountResult.reason || '')}`);
            }
            // 결과 추가
            results.push({
                keyword: originalKeyword, // 원본 키워드 유지
                pcSearchVolume: pcVolume,
                mobileSearchVolume: mobileVolume,
            ...
            console.error(`[NAVER-VOLUME] ❌ 키워드 "${keyword}" 처리 실패:`, errorMessage);
            // 에러 발생 시에도 기본값 반환
            results.push({
                keyword: keyword,
                pcSearchVolume: 0,
 ...
 */
async function getNaverKeywordSearchVolume(config, keywords) {
 ...
 */
async function getNaverRelatedKeywords(baseKeyword, config, options = {}) {
    if (!config.clientId || !config.clientSecret) {
        throw new Error('네이버 API 인증 정보가 필요합니다');
    }
    ...
    try {
        ...
        const { getNaverAutocompleteKeywords } = await Promise.resolve().then(() => __importStar(require('./naver-autocomplete')));
        ...
        const smartBlockKeywords = new Set();
        try {
        ...
        const mindMapKeywords = new Set();
        // 🕷️ 거미줄 치기: 조금이라도 연상되는 키워드 찾기
        const spiderWebKeywords = new Set();
        // 제품/음식 관련 키워드 마인드맵
        ...
        // 🕷️ 거미줄 치기: SEO 모드에서는 비활성화 (불필요한 API 호출 방지)
        // 먼저 키워드 포함 연관키워드를 수집한 후, 그 키워드들로 다시 검색하여 거미줄처럼 확장
        const spiderWebDepth = 0; // 비활성화 (원래 2단계)
        const spiderWebProcessed = new Set([baseKeyword]);
        const spiderWebQueue = [baseKeyword];
        console.log(`[NAVER-RELATED] ℹ️  거미줄 치기 건너뜀 (SEO 모드에서는 비활성화)`);
        for (let depth = 0; depth < spiderWebDepth && spiderWebQueue.length > 0; depth++) {
            ...
            for (const currentKeyword of currentLevelKeywords) {
                if (spiderWebProcessed.has(currentKeyword))
                    continue;
        ...
        if (uniqueItems.length > 0) {
            ...
            uniqueItems.forEach((item) => {
                ...
                if (title.includes(baseKeyword)) {
                    ...
                    const titleWords = title.split(/[\s|,，、·\[\]()【】「」<>]+/).filter((w) => w.trim().length > 0);
                ...
                const getRelevantSuffixes = (keyword) => {
                ...
                const isValidCombination = (keyword, suffix) => {
                ...
                relevantSuffixes.forEach(suffix => {
                    if (isValidCombination(baseKeyword, suffix)) {
                        const combined = `${baseKeyword} ${suffix}`.trim();
                        if (combined.length >= baseKeyword.length && combined.length <= 25) {
                            extractedKeywords.add(combined);
                        }
                    }
                });
            });
            // 🔧 개선: 제목에서 더 많은 키워드 추출 - 키워드와 함께 자주 나오는 단어들
            const keywordCombinations = new Set();
            const keywordFrequency = new Map(); // 키워드 빈도 추적
            ...
            uniqueItems.forEach((item) => {
                ...
                if (title.includes(baseKeyword)) {
                    ...
                    const words = title.split(/[\s|,，、·\[\]()【】「」<>]+/).filter((w) => w.trim().length > 0);
                    // 키워드의 위치 찾기
                    const keywordIndex = words.findIndex((w) => w.includes(baseKeyword));
                    if (keywordIndex >= 0) {
                        // 키워드 앞 단어들 조합
                        if (keywordIndex > 0) {
            ...
            const allCollectedKeywords = new Set();
            ...
            const uniqueKeywords = Array.from(new Set(filteredKeywords)).sort((a, b) => {
                ...
                if (aInAutocomplete && !bInAutocomplete)
                    return -1;
                if (!aInAutocomplete && bInAutocomplete)
                    return 1;
