 ...
 */
export async function validateLink(url: string, timeout: number = 5000): Promise<{
  isValid: boolean;
  statusCode?: number;
  error?: string;
}> {
 ...
 */
export async function getValidNaverLinks(
  keyword: string,
  credentials: { clientId: string; clientSecret: string },
  options: {
    maxResults?: number;
  ...
  const {
    maxResults = 10,
    maxConcurrent = 5,
    validateLinks: shouldValidate = true
  } = options;
  ...
  try {
    ...
    if (shouldValidate) {
      ...
      const validatedLinks = await validateLinks(
        searchResults.map(link => ({ url: link.link, title: link.title, description: link.description })),
        maxConcurrent
      );
 ...
 */
export async function getValidLinksForMultipleKeywords(
  keywords: string[],
  credentials: { clientId: string; clientSecret: string },
  options: {
