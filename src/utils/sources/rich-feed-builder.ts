...
const FOREIGN_CELEB_RE = /(호소키\s*카즈코|호소키|카즈코|토미오|패리스\s*잭슨|마이클\s*잭슨|자파\s*잭슨|저메인\s*잭슨|티토\s*잭슨|랜디\s*잭슨|재키\s*잭슨|블랭킷\s*잭슨|프린스\s*잭슨|리사\s*마리\s*프레슬리|리사\s*마리|엘비스\s*프레슬리|프레슬리|엘론\s*머스크|일론\s*머스크|트럼프|바이든|푸틴|시진핑|아베|기시다|키미짱|아이코|미사키|나오|유키|아유미|마호|히카루|타로|이치로|히로키|쇼타|토모|아키라|료|켄|진|렌|루이|히로|준|쇼|마사|토시|타카시|쇼고|히로토|류|레이|소우|타쿠야|미야|유토|소라|료타|료스케|미나|아키|토모코|에리코|에리|미사토|미와|미키|미카|마이|미카코|미즈키|아카네|에리카|미나미|에미)/i;
