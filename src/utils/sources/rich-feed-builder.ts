...
const FINANCE_EXPERT_RE = /(정기예금|적금|예금|대출|주식|펀드|연금|보험|투자|채권|ETF|증권|금리|이율|이자율|배당|시세|환율|재테크|증여세|상속세|소득세|법인세|공제|면세|세무|회계|연말정산|종합소득세|부가가치세|양도세|취득세|등록세|재산세|국민연금|건강보험|고용보험|산재보험)/;
...
// v2.43.20: 외국 셀럽/회사명/약품명/영문 단일토큰 추가 차단
const FOREIGN_CELEB_RE = /^(호소키|토미오|패리스\s*잭슨|마이클\s*잭슨|엘론|트럼프|바이든|푸틴|시진핑|키미짱|아이코|미사키|나오|유키|아유미|마호|히카루|타로|이치로|히로키|쇼타|토모|아키라|료|켄|진|렌|루이|히로|준|쇼|마사|토시|타카시|쇼고|히로토|류|레이|소우|타쿠야|미야|유토|소라|료타|료스케|히카루|미나|아키|토모코|에리코|에리|미사토|미와|미키|미카|아유미|에리코|마이|미카코|미즈키|아카네|에리카|미나미|에미)\b/i;
const COMPANY_BRAND_NAME_RE = /(컴퍼니|코퍼레이션|코퍼레이트|주식회사|유한회사|holdings|corporation|company|inc\.|ltd\.|코리아|글로벌|인터내셔널|엔터프라이즈|솔루션|시스템즈|네트웍스|테크놀로지)/i;
...
function calculateBloggerWritability(keyword: string, docCount: number, searchVolume: number): number {
    ...
    // v2.43.20: 외국 셀럽 (호소키 카즈코, 패리스 잭슨 등) — KOREAN_SURNAMES 못 잡는 사례
    if (FOREIGN_CELEB_RE.test(clean)) score -= 30;
    // 회사명 / 브랜드 (여기어때컴퍼니 등) — 회사 정보 글은 일반 블로거 도메인 아님
    if (COMPANY_BRAND_NAME_RE.test(clean)) score -= 25;
    // 약품명 (아티반 등) — 약사/의사 전용
    if (DRUG_NAME_RE.test(clean)) score -= 30;
    // 영문 단일토큰 (CORTIS 등) — 그룹명/제품명 등 한국 블로거가 글감 잡기 어려움
    if (isEnglishSingleToken(clean)) score -= 22;
