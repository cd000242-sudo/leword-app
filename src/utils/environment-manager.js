 ...
 * 네이버 API 연동 테스트
 */
async function testNaverApiConnection(clientId, clientSecret) {
 ...
 * 대량 크롤링 시스템 테스트
 */
async function testMassCrawlingSystem() {
    try {
        ...
        // 실제 크롤링 테스트
        const { MassCrawlingSystem } = await Promise.resolve().then(() => __importStar(require('../core/mass-crawler')));
        const crawler = new MassCrawlingSystem(config.naverClientId, config.naverClientSecret, config.googleApiKey, config.googleCseId);
