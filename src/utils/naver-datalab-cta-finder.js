...
exports.findCtaLinksViaDatalab = findCtaLinksViaDatalab;
const naver_datalab_api_1 = require("./naver-datalab-api");
const naver_search_validator_1 = require("./naver-search-validator");
/**
 * 네이버 데이터랩 API를 활용하여 CTA 링크 후보 발굴
 */
async function findCtaLinksViaDatalab(topic, keywords, config, options = {}) {
    const { maxResults = 10, minRelevance = 50, includeTrendKeywords = true, includeRelatedKeywords = true, validateLinks = true } = options;
    const candidates = [];
    ...
    try {
        ...
        if (includeTrendKeywords) {
            ...
            try {
                const trendKeywords = await (0, naver_datalab_api_1.getNaverTrendKeywords)(config, {
                ...
                for (const trend of trendKeywords.slice(0, 5)) {
                    if (trend.keyword && trend.searchVolume && trend.searchVolume > 0) {
                        try {
                            const links = await (0, naver_search_validator_1.getValidNaverLinks)(trend.keyword, config, {
                                maxResults: 3,
        ...
        if (includeRelatedKeywords) {
            ...
            try {
                ...
                for (const baseKeyword of searchKeywords) {
                    try {
                        const relatedKeywords = await (0, naver_datalab_api_1.getNaverRelatedKeywords)(baseKeyword, config, {
                            limit: 5,
                            page: 0
                        });
                        // 연관 키워드로 검색하여 링크 찾기
                        for (const related of relatedKeywords.slice(0, 3)) {
                            if (related.keyword && related.keyword !== baseKeyword) {
                                try {
                                    const links = await (0, naver_search_validator_1.getValidNaverLinks)(related.keyword, config, {
                                        maxResults: 2,
                                        validateLinks: validateLinks
                                    });
                                    for (const link of links) {
        ...
        for (const keyword of directKeywords) {
            try {
                const links = await (0, naver_search_validator_1.getValidNaverLinks)(keyword, config, {
                    maxResults: 3,
                    validateLinks: validateLinks
                });
                for (const link of links) {
 ...
 * 관련성 점수 계산 (0-100)
 */
function calculateRelevance(topic, keywords, foundKeyword, linkTitle) {
    let score = 0;
    // 1. 주제와의 일치도 (40점)
    ...
    if (titleLower.includes(topicLower) || foundLower.includes(topicLower)) {
        ...
        const matchCount = topicWords.filter(word => titleLower.includes(word) || foundLower.includes(word)).length;
