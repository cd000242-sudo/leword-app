...
(async () => {
  ...
  assert(
    ...
    !(__liveGoldenRadarTestInternals as any).hasRecentCacheZeroSplitProbe({
      splitProbeZeroAt: '2026-07-15T00:30:00.000Z',
      evidence: ['searchad-pc-mobile-split-zero'],
    }, splitProbeNowMs),
  );
  assert(
    'current zero-split tombstones still enforce the seven-day retry cooldown',
    (__liveGoldenRadarTestInternals as any).hasRecentCacheZeroSplitProbe({
      splitProbeZeroAt: '2026-07-15T00:30:00.000Z',
      evidence: ['searchad-pc-mobile-split-zero:v2-exact-fallback'],
    }, splitProbeNowMs),
  );
  ...
  fs.writeFileSync(extraFieldsBoardFile, JSON.stringify({
    ...
    items: [{
      ...
      searchVolumeConfidence: 'high',
      searchVolumeBindingVersion: 'keyword-keyed-v2',
      searchVolumeMeasuredAt: extraFieldsStamp,
      isSearchVolumeEstimated: false,
      documentCountSource: 'naver-api',
      documentCountConfidence: 'high',
      documentCountQueryMode: 'broad',
  ...
  const legacyExactBoardFile = path.join(process.cwd(), 'tmp', 'mobile-live-golden-legacy-exact-test.json');
  const legacyExactStamp = new Date().toISOString();
  const legacyExactRow = (keyword: string, evidence: string[]) => ({
    keyword,
    grade: 'SS' as const,
    score: 82,
    ...
    searchVolumeConfidence: 'high',
    searchVolumeBindingVersion: 'keyword-keyed-v2',
    searchVolumeMeasuredAt: legacyExactStamp,
    isSearchVolumeEstimated: false,
    documentCountSource: 'naver-api',
    documentCountConfidence: 'high',
    isDocumentCountEstimated: false,
    updatedAt: legacyExactStamp,
    discoveredAt: legacyExactStamp,
    isMeasured: true,
  });
  fs.writeFileSync(legacyExactBoardFile, JSON.stringify({
    boardUpdatedAt: legacyExactStamp,
    items: [
      legacyExactRow('에어컨이전설치비용', [
        'naver-openapi-exact-phrase',
        'direct-searchad-exact-measured',
      ]),
      legacyExactRow('냉장고수리비용', ['naver-openapi-document-count']),
    ],
  }), 'utf8');
  const legacyExactRadar = new MobileLiveGoldenRadar({
    notificationInbox: inbox,
    runOnStart: false,
    cycleLimit: 1,
    boardTarget: 10,
    maxCandidates: 60,
    boardFile: legacyExactBoardFile,
    categories: ['home_life'],
    enableBackfill: false,
    discover: async () => [],
  });
  const legacyExactSnapshot = legacyExactRadar.snapshot();
  const legacyExactPersistedItems = [...((legacyExactRadar as any).board.values() as Iterable<any>)];
  const legacyExactItem = legacyExactPersistedItems.find((item) => item.keyword === '에어컨이전설치비용');
  const unprovenLegacyItem = legacyExactPersistedItems.find((item) => item.keyword === '냉장고수리비용');
  assert('legacy exact-phrase evidence alone never restores a missing query mode',
    !!legacyExactItem
      && legacyExactItem.documentCountQueryMode === undefined
      && !legacyExactSnapshot.verifiedSupply.some((item) => item.keyword === legacyExactItem.keyword),
    JSON.stringify(legacyExactItem));
  assert('legacy rows without explicit exact-phrase evidence remain unclassified',
    !!unprovenLegacyItem
      && unprovenLegacyItem.documentCountQueryMode === undefined
      && !legacyExactSnapshot.verifiedSupply.some((item) => item.keyword === unprovenLegacyItem.keyword),
    JSON.stringify(unprovenLegacyItem));
  fs.rmSync(legacyExactBoardFile, { force: true });
  ...
  const realDemandRow = (keyword: string, index: number) => ({
    ...
    searchVolumeConfidence: 'high',
    searchVolumeBindingVersion: 'keyword-keyed-v2',
    searchVolumeMeasuredAt: realDemandStamp,
    isSearchVolumeEstimated: false,
    documentCountSource: 'naver-api',
    documentCountConfidence: 'high',
    documentCountQueryMode: 'broad',
  ...
  const realDemandRadar = new MobileLiveGoldenRadar({
    ...
    measureLiveDocumentCount: async (keyword) => ({
      dc: keyword.includes('200\uCC28') ? 600 : 610,
      source: 'naver-api',
      confidence: 'high',
      isEstimated: false,
      queryMode: 'broad',
  ...
  const surgeRadar = new MobileLiveGoldenRadar({
    ...
    realDemandProbe: async (query: string) => {
      ...
      if (query === '김부장') {
        return surgeCycle === 1
          ? { ok: true, suggestions: ['김부장 기본정보', '김부장 등장인물', '김부장 시청률', '김부장 방송일정'] }
          : { ok: true, suggestions: ['김부장 기본정보', '김부장 등장인물', '김부장 시청률', '김부장 방송일정', '김부장 결말 해석'] };
      }
      if (query === '김부장 결말 해석') {
  ...
  assert('traffic-surge product rows stay outside Phase 1C verified supply',
    surgeRows.length > 0
      && !(surgeSnapshot.verifiedSupply || []).some((item) => item.lane === 'traffic-surge'),
    JSON.stringify(surgeSnapshot.verifiedSupply || []));
  ...
  assert('traffic-surge preserves canonical broad document provenance and rejects numeric cache rows',
    surgeRows.length > 0
      && surgeRows.every((item) => (
        item.documentCountSource === 'naver-api'
        && item.documentCountConfidence === 'high'
        && item.documentCountQueryMode === 'broad'
        && item.documentCountMeasuredAt === surgeDocumentMeasuredAt
        && item.isDocumentCountEstimated === false
      ))
      && !surgeRows.some((item) => item.keyword === '김부장 방송일정'),
    JSON.stringify(surgeRows.map((item) => ({
      keyword: item.keyword,
      source: item.documentCountSource,
      confidence: item.documentCountConfidence,
  ...
  const surgeReloadSnapshot = surgeReloadRadar.snapshot();
  assert('reloaded traffic-surge rows stay outside Phase 1C verified supply',
    surgeReloadSnapshot.board.some((item) => item.lane === 'traffic-surge')
      && !(surgeReloadSnapshot.verifiedSupply || []).some((item) => item.lane === 'traffic-surge'),
    JSON.stringify(surgeReloadSnapshot.verifiedSupply || []));
  ...
  assert('ingest accepts only measured non-estimated rows',
    ingestResult.received === 4 && ingestResult.accepted === 1 && ingestResult.persisted === true,
  ...
  assert('ingested row reaches read-only snapshot through inbox merge with measured extras',
    !!ingestedItem
      ...
      && ingestedItem.documentCountQueryMode === 'broad'
      && Number.isFinite(Date.parse(String(ingestedItem.documentCountMeasuredAt || ''))),
    JSON.stringify({
  ...
  assert('canonical OpenAPI broad backfill enters verified supply without opening a second direct SearchAd lane',
    ...
    JSON.stringify({
      ...
      internalBoard: backfillCatchupInternalBoard.map((item: any) => ({
        keyword: item.keyword,
        source: item.documentCountSource,
        mode: item.documentCountQueryMode,
  ...
  assert('broad scrape autocomplete longtails stay rejected from verified and publishable supply',
    autocompleteBackfillVolumeCalls > 0
      ...
      && autocompleteBackfillInternalBoard.every((item: any) => (
        !isTrustedLiveGoldenSupplyRow(item)
        && item.publishDecision?.verdict !== 'publish'
        && item.documentCountSource !== 'naver-api'
      ))
    ...
    JSON.stringify({
      ...
      internalBoard: autocompleteBackfillInternalBoard.map((item: any) => item.keyword),
    }));
  ...
  const measuredProbeDocumentOptions: Array<{
    keyword: string;
    scrapeOnly?: boolean;
    queryMode?: string;
  }> = [];
  const measuredProbeRadar = new MobileLiveGoldenRadar({
    ...
    measureLiveDocumentCount: async (keyword, options) => {
      measuredProbeDocumentOptions.push({
        keyword,
  ...
  assert('broad scrape measured-probe documents stay rejected from verified and publishable supply',
    measuredProbeVolumeCalls > 0
      ...
      && measuredProbeDocumentOptions.some((item) => (
        ...
        && item.queryMode === 'broad'
      ))
      && measuredProbeSnapshot.board.length === 0
      && measuredProbeInternalBoard.every((item: any) => (
        !isTrustedLiveGoldenSupplyRow(item)
        && item.publishDecision?.verdict !== 'publish'
        && item.documentCountSource !== 'naver-api'
      )),
    JSON.stringify({
      ...
      internalBoard: measuredProbeInternalBoard.map((item: any) => `${item.keyword}:${item.documentCountSource}:${item.documentCountQueryMode || 'missing'}`),
  ...
  const noisyProbeInternalBoard = [
    ...((noisyProbeRadar as any).board.values() as Iterable<any>),
  ];
  const noisyAutocompleteSuggestionIds = new Set(
    noisyAutocompleteSuggestions.map((keyword) => keyword.replace(/\s+/g, '')),
  );
  assert('measured probe candidates outrank noisy autocomplete candidates while scrape rows are discarded',
    noisyProbeVolumeKeywords.length >= 24
      ...
      && noisyProbeSnapshot.board.length === 0
      && noisyProbeInternalBoard.length === 0,
    JSON.stringify({
      ...
      internalBoard: noisyProbeInternalBoard.map((item: any) => item.keyword),
    }));
  ...
  const liveIssueMeasuredFallbackInternalBoard = [
    ...((liveIssueMeasuredFallbackRadar as any).board.values() as Iterable<any>),
  ];
  assert('broad live-issue scrape fallback is discarded from verified and internal supply',
    measuredFallbackVolumeCalls > 0
      && liveIssueMeasuredFallbackSnapshot.board.length === 0
      && liveIssueMeasuredFallbackInternalBoard.length === 0,
  ...
  assert('discarded live issue scrape rows cannot preserve publishable split metrics',
    !liveIssueMeasuredFallbackInternalBoard.some((item: any) => (
      item.keyword === '\uC18C\uC0C1\uACF5\uC778 \uD2B9\uB840\uBCF4\uC99D \uC2E0\uCCAD \uBC29\uBC95'
      && isTrustedLiveGoldenSupplyRow(item)
    )),
    liveIssueMeasuredFallbackInternalBoard.map((item: any) => `${item.keyword}:${item.pcSearchVolume}:${item.mobileSearchVolume}:${item.cpc}`).join('|'));
  ...
  fs.writeFileSync(staleBoardFile, JSON.stringify({
    ...
    items: [
      ...
      },
      {
        ...
        searchVolumeConfidence: 'high',
        searchVolumeBindingVersion: 'keyword-keyed-v2',
        searchVolumeMeasuredAt: '2026-06-13T08:20:00.000Z',
        isSearchVolumeEstimated: false,
   documentCountSource: 'naver-api',
   documentCountConfidence: 'high',
    documentCountQueryMode: 'broad',
  ...
  fs.writeFileSync(zeroScorePreviewBoardFile, JSON.stringify({
    ...
    items: [
      ...
      searchVolumeConfidence: 'high',
      searchVolumeBindingVersion: 'keyword-keyed-v2',
      searchVolumeMeasuredAt: '2026-06-13T08:59:00.000Z',
      isSearchVolumeEstimated: false,
   documentCountSource: 'naver-api',
   documentCountConfidence: 'high',
    documentCountQueryMode: 'broad',
  ...
  fs.writeFileSync(proGapBoardFile, JSON.stringify({
    ...
    items: [
      ...
      searchVolumeConfidence: 'high',
      searchVolumeBindingVersion: 'keyword-keyed-v2',
      searchVolumeMeasuredAt: '2026-06-13T08:50:00.000Z',
      isSearchVolumeEstimated: false,
   documentCountSource: 'naver-api',
   documentCountConfidence: 'high',
    documentCountQueryMode: 'broad',
  ...
  fs.writeFileSync(previewLeakBoardFile, JSON.stringify({
    ...
    items: [
      ...
      searchVolumeConfidence: 'high',
      searchVolumeBindingVersion: 'keyword-keyed-v2',
      searchVolumeMeasuredAt: index < 3 ? '2026-06-13T08:55:00.000Z' : '2026-06-10T08:55:00.000Z',
      isSearchVolumeEstimated: false,
     documentCountSource: 'naver-api',
     documentCountConfidence: 'high',
      documentCountQueryMode: 'broad',
  ...
  fs.writeFileSync(categoryDiversityBoardFile, JSON.stringify({
    ...
    items: categoryDiversityRows.map(([keyword, grade, score, totalSearchVolume, documentCount, goldenRatio, category]) => ({
      ...
      searchVolumeConfidence: 'high',
      searchVolumeBindingVersion: 'keyword-keyed-v2',
      searchVolumeMeasuredAt: '2026-06-13T08:58:00.000Z',
      isSearchVolumeEstimated: false,
     documentCountSource: 'naver-api',
     documentCountConfidence: 'high',
      documentCountQueryMode: 'broad',
  ...
  fs.writeFileSync(measuredOnlyBoardFile, JSON.stringify({
    ...
    items: measuredOnlyRows.map(([keyword, grade, score, totalSearchVolume, documentCount, goldenRatio, category, isMeasured]) => ({
      ...
      searchVolumeBindingVersion: Number(totalSearchVolume) > 0 ? 'keyword-keyed-v2' : undefined,
      searchVolumeMeasuredAt: Number(totalSearchVolume) > 0 ? '2026-06-13T09:01:00.000Z' : undefined,
      isSearchVolumeEstimated: false,
  ...
  fs.writeFileSync(contentDiversityBoardFile, JSON.stringify({
    ...
    items: contentDiversityRows.map(([keyword, grade, score, totalSearchVolume, documentCount, goldenRatio, category]) => ({
      ...
      searchVolumeConfidence: 'high',
      searchVolumeBindingVersion: 'keyword-keyed-v2',
      searchVolumeMeasuredAt: '2026-06-13T09:03:00.000Z',
      isSearchVolumeEstimated: false,
     documentCountSource: 'naver-api',
     documentCountConfidence: 'high',
      documentCountQueryMode: 'broad',
  ...
  const exactFallbackIntentRejected = !__liveGoldenRadarTestInternals.isMeasuredExactDisplayFallbackMetric({
    keyword: '\uAD6D\uBBFC\uB0B4\uC77C\uBC30\uC6C0\uCE74\uB4DC\uC0AC\uC6A9\uCC98',
    grade: 'A',
    score: 71,
    pcSearchVolume: 1360,
  ...
  const exactFallbackPromotionRejectsPendingSplit = !__liveGoldenRadarTestInternals.isMeasuredExactDisplayPromotionCandidate({
    keyword: '\uC81C\uC8FC\uACF5\uD56D\uB80C\uD2B8\uCE74\uCD94\uCC9C',
    grade: 'A',
    score: 65,
    pcSearchVolume: 0,
  ...
  assert('cache/medium exact document fallbacks stay rejected regardless of intent or split state',
    exactFallbackIntentRejected
      && exactFallbackCalculatorRejected
      && exactFallbackNeedsPcMobileSplit
      && exactFallbackPromotionRejectsPendingSplit,
    JSON.stringify({
      exactFallbackIntentRejected,
      exactFallbackCalculatorRejected,
      exactFallbackNeedsPcMobileSplit,
  ...
  const referenceProbeInternalBoard = [
    ...((referenceProbeRadar as any).board.values() as Iterable<any>),
  ];
  assert('scrape-measured broad references are discarded from verified and internal supply',
    referenceProbeMeasuredKeywords.includes(referenceProbeTarget)
      && !referenceProbeSnapshot.board.some((item) => item.keyword === referenceProbeTarget)
      && referenceProbeInternalBoard.every((item: any) => item.keyword !== referenceProbeTarget),
    JSON.stringify({
      ...
      internalBoard: referenceProbeInternalBoard.map((item: any) => `${item.keyword}:${item.documentCountSource}:${item.documentCountQueryMode || 'missing'}`),
    ...
    items: [
      {
        ...
        searchVolumeConfidence: 'high',
        searchVolumeBindingVersion: 'keyword-keyed-v2',
        searchVolumeMeasuredAt: '2026-06-21T17:04:21.000Z',
        isSearchVolumeEstimated: false,
     documentCountSource: 'naver-api',
     documentCountConfidence: 'high',
      documentCountQueryMode: 'broad',
  ...
  const underfilledVisibleItem = {
    ...
    documentCountMeasuredAt: '2026-07-14T00:59:00.000Z',
  ...
  const underfilledCacheRadar = new MobileLiveGoldenRadar({
    ...
    measureLiveSearchVolumeSeparate: async (_config, keywords, options) => {
      ...
      return keywords.map((keyword) => ({
        ...
        searchVolumeConfidence: 'high',
        searchVolumeBindingVersion: 'keyword-keyed-v2',
        searchVolumeMeasuredAt: '2026-07-14T01:00:00.000Z',
        isSearchVolumeEstimated: false,
      }));
    },
  });
  ...
  const queueVariantInternalBoard = [
    ...((queueVariantRadar as any).board.values() as Iterable<any>),
  ];
  const queueVariantAfter = JSON.parse(fs.readFileSync(queueVariantProbeFile, 'utf8'));
  assert('spaced legacy probes preserve measurement text but discard scrape-only results',
    queueVariantMeasuredKeywords.includes(spacedTravelProbe)
      && queueVariantMeasuredKeywords.length <= 80
      && !queueVariantSnapshot.board.some((item) => item.keyword === spacedTravelProbe)
      && queueVariantInternalBoard.every((item: any) => item.keyword !== spacedTravelProbe),
    JSON.stringify({
      ...
      internalBoard: queueVariantInternalBoard.map((item: any) => `${item.keyword}:${item.documentCountSource}:${item.documentCountQueryMode || 'missing'}`),
  ...
  const volumeBatchInternalBoard = [
    ...((volumeBatchRadar as any).board.values() as Iterable<any>),
  ];
  assert('live golden volume measurement keeps small batches while discarding scrape-only results',
    volumeBatchSizes.length >= 2
      && volumeBatchSizes.every((size) => size <= 4)
      && !volumeBatchSnapshot.board.some((item) => item.keyword === volumeBatchWinner)
      && volumeBatchInternalBoard.every((item: any) => item.keyword !== volumeBatchWinner),
    JSON.stringify({
      volumeBatchSizes,
      ...
      internalBoard: volumeBatchInternalBoard.map((item: any) => `${item.keyword}:${item.documentCountSource}:${item.documentCountQueryMode || 'missing'}`),
    }));
  ...
  assert('pro live golden board refuses filler while keeping a large trusted measured display set',
    proMeasuredDisplaySnapshot.board.length >= 40
      ...
      && proMeasuredDisplaySnapshot.board.every((item) => ['publish', 'conditional'].includes(String(item.publishDecision?.verdict)))
      && proMeasuredDisplaySnapshot.board.every((item) => item.publishDecision?.verdict !== 'exclude')
    ...
    items: [
      {
        ...
        searchVolumeConfidence: 'high',
        searchVolumeBindingVersion: 'keyword-keyed-v2',
        searchVolumeMeasuredAt: '2026-07-01T08:10:00.000Z',
        isSearchVolumeEstimated: false,
       documentCountSource: 'naver-api',
       documentCountConfidence: 'high',
        documentCountQueryMode: 'broad',
      ...
      },
      {
        ...
        searchVolumeConfidence: 'high',
        searchVolumeBindingVersion: 'keyword-keyed-v2',
        searchVolumeMeasuredAt: '2026-07-01T08:20:00.000Z',
        isSearchVolumeEstimated: false,
       documentCountSource: 'naver-api',
       documentCountConfidence: 'high',
        documentCountQueryMode: 'broad',
  ...
  fs.writeFileSync(searchAdQuotaBoardFile, JSON.stringify({
    ...
    items: [{
      ...previewBoardItem('청년미래적금 가입신청 대상', 'policy', 0),
      searchVolumeMeasuredAt: '2026-07-11T09:59:00.000Z',
  ...
  const quotaScrapeRadar = new MobileLiveGoldenRadar({
    ...
    measureLiveDocumentCount: async (_keyword, options) => {
      ...
      return {
        ...
        isEstimated: false,
        queryMode: 'broad',
        measuredAt: new Date(quotaScrapeNowMs).toISOString(),
        debug: { scrapeDc: 360 },
      };
    },
  ...
  const quotaScrapeInternalBoard = [
    ...((quotaScrapeRadar as any).board.values() as Iterable<any>),
  ];
  assert('OpenAPI quota fallback discards scrape documents from verified and internal supply',
    quotaScrapeVolumeCalls > 0
      && quotaScrapeDocumentCalls > 0
      && quotaScrapeSnapshot.skippedRuns === 0
      && quotaScrapeSnapshot.board.length === 0
      && quotaScrapeInternalBoard.length === 0,
    JSON.stringify({
      ...
      internalBoard: quotaScrapeInternalBoard,
    }));
  ...
  const schedulerCatchupRadar = new MobileLiveGoldenRadar({
    ...
    enableBackfill: false,
    setIntervalFn: (handler) => {
      catchupTicks.push(handler);
      return `catchup-timer-${catchupTicks.length}`;
    },
    clearIntervalFn: () => {},
    measureLiveSearchVolumeSeparate: async () => [],
    discover: async () => {
  ...
  catchupTicks.forEach((handler) => handler());
  for (let attempt = 0; attempt < 100 && schedulerCatchupDiscoverCalls < 2; attempt += 1) {
    ...
    items: [
      {
       ...
       isDocumentCountEstimated: false,
        ...
        searchVolumeConfidence: 'high',
        searchVolumeBindingVersion: 'keyword-keyed-v2',
        searchVolumeMeasuredAt: '2026-06-28T02:59:00.000Z',
        isSearchVolumeEstimated: false,
