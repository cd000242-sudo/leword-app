...
describe('제목이 어느 갈래인지 글자로 가린다', () => {
  it('검색어로 시작하면 검색용', () => {
    expect(titleTargetOf('계약학과 지원이 몰린 이유', KW)).toBe('설명');
    expect(titleTargetOf('반도체 계약학과 뽑는 기업과 조건', KW)).toBe('설명');
  });

  it('검색어를 문장 한가운데 묻으면 검색용이 아니다 — 이번 사고의 핵심', () => {
    expect(titleTargetOf('왜 반도체 계약학과로 지원이 쏠렸을까', KW)).not.toBe('설명');
    expect(titleTargetOf('의대 대신 반도체 계약학과 고른 수험생들', KW)).not.toBe('설명');
  });

  it('답을 요구하는 꼴로 끝나고 검색어가 앞쪽에 있으면 AI답변용', () => {
    expect(titleTargetOf('계약학과 지원 자격은 어떻게 되는지', KW)).toBe('질문');
    expect(titleTargetOf('계약학과 취업 연계는 어디까지인가', KW)).toBe('질문');
    expect(titleTargetOf('계약학과 원서접수 언제', KW)).toBe('질문');
  });

  it('질문 꼴이어도 검색어가 뒤쪽에 있으면 AI답변용이 아니다', () => {
    expect(titleTargetOf('올해 수험생들이 몰린 곳이 반도체 계약학과인가', KW)).not.toBe('질문');
  });

  it('숫자가 박혀 있으면 인용용', () => {
    expect(titleTargetOf('계약학과 2027학년도 수시 마감 결과', KW)).toBe('수치');
    expect(titleTargetOf('반도체 계약학과 6장 지원 흐름', KW)).toBe('수치');
  });
...
describe('검증기가 갈래 없는 제목을 떨어뜨린다', () => {
  ...
  it('세 갈래를 갖춘 것만 남고, 라벨이 붙는다', () => {
    ...
    expect(new Set(got.map((t) => t.target))).toEqual(new Set(['설명', '질문', '수치']));
  ...
  it('갈래가 고루 섞인다 — 한 갈래가 자리를 다 먹지 않는다', () => {
    ...
    expect(got.filter((t) => t.target === '질문').length).toBe(1);
    expect(got.filter((t) => t.target === '설명').length).toBe(2);
  });
...
describe('프롬프트가 세 갈래를 실제로 청한다', () => {
  ...
  it('target 필드를 요구한다', () => {
    expect(prompt).toContain('"target":"질문"');
  });

  it('세 갈래 규칙이 다 적혀 있다', () => {
    expect(prompt).toContain('설명|질문|수치');
    expect(prompt).toContain('성과가 아닌 문장 형태');
    expect(prompt).toContain('대상 검색어로 시작');
  });

  it('AI 티 금지 교리는 그대로 남아 있다', () => {
    expect(prompt).toContain('총정리');
    expect(prompt).toContain('확인된 사실');
    expect(prompt).toContain('답을 숨기기 위해 사실을 왜곡하지 마라');
  });
});
