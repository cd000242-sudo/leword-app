...
// AGENTS.md 통일 등급: 저볼륨 winnable은 SSS 우회 승격하지 않는다.
assert('winnable: low-vol high-ratio does not enter SSS', isWinnableSssMetrics(800, 200, 4) === false);
...
assert('golden = classic only', isGoldenSssMetrics(1200, 200, 6) === true && isGoldenSssMetrics(800, 200, 4) === false);

// === 통합: 등급 라우트 (하위 SS/S/A/B cascade 유지) ===
assert('liveGradeFromMetrics keeps low-vol winnable at SS',
  liveGradeFromMetrics(82, 800, 200, 4, '주휴수당 신청방법') === 'SS',
...
assert('floor module rejects low-vol winnable SSS row',
  isStrictGoldenDiscoverySss({
    keyword: '주휴수당 신청방법',
    grade: 'SSS',
    score: 82,
...
assert('floor module accepts classic SSS row',
  isStrictGoldenDiscoverySss({
    keyword: '청년 주거급여 신청자격',
    grade: 'SSS',
    score: 88,
    searchVolume: 1200,
    documentCount: 300,
