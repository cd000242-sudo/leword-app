...
describe('뉴스 제목 머리말', () => {
    ...
    /*
     * 네 번째 감사(34177425968)의 실제 뉴스에서 머리말로 새어 나온 것들 — 순위·수치·강조어.
     * '최고'·'1위'·'천만'·'대상'·'함께'·'출격'·'매진'·'2관왕'·'커리어하이'·'건강이상'·'활동중단'.
     * 낱말 목록만으론 끝이 없어 꼴로도 막는다: 숫자+위/관왕, ~시청률·~산업·~모임·~저하·~이상.
     */
    it('실제 뉴스에서 새어 나온 순위·수치·강조어는 머리말이 아니다', () => {
        const raw = [
            "'최고' 시청률 경신…'1위' 등극", "'최고' 화제작 '1위' 굳히기", "'천만' 돌파 '대상' 수상", "'천만' 관객 '대상' 영예",
            "'함께' 출연 '출격'", "'함께' 무대 '출격' 예고", "'매진' 행렬 '2관왕'", "'매진' 사례 '2관왕' 달성",
            "'커리어하이' 기록", "'커리어하이' 경신", "'건강이상'설 부인", "'건강이상' 없다", "'활동중단' 선언", "'활동중단' 이유",
            "'최고시청률' 갱신", "'최고시청률' 20%", "'영상산업' 진흥", "'영상산업' 회의",
        ];
        const heads = extractTitleHeads(raw, { minCount: 2, limit: 30 });
        for (const noise of ['최고', '1위', '천만', '대상', '함께', '출격', '매진', '2관왕', '커리어하이', '건강이상', '활동중단', '최고시청률', '영상산업']) {
            expect(heads, `${noise} 가 머리말로 나왔다`).not.toContain(noise);
        }
    });

    /*
     * 받아들일지 밖에서 정한다(accept). 연예 기사에 따옴표로 든 병명('미주신경기능저하')이
     * 방송 머리말이 되어 그 연관어가 방송에 실렸다(실측). 말 규칙이 딴 주제라고 하면 머리말에서
     * 빼는데, **상한을 자르기 전에** 빼야 자리를 안 잡아먹는다.
     */
    it('accept 가 거른 후보는 상한 자리를 차지하지 않는다', () => {
        const raw = [
            "'미주신경기능저하' 진단", "'미주신경기능저하' 휴식", "'폭싹 속았수다' 종영", "'폭싹 속았수다' 눈물",
            "'인턴' 합류", "'인턴' 첫 출근",
        ];
        const heads = extractTitleHeads(raw, { minCount: 2, limit: 1, accept: (head) => !/미주신경|인턴/.test(head) });
        expect(heads).toEqual(['폭싹속았수다']);
    });
