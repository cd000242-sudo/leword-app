...
describe('봉투를 못 만들었을 때', () => {
  ...
  it('첫 페이지는 못 들었어도 30위 안 기록이 있으면 오늘 찾을 범위를 말한다(2026-09-15)', () => {
    expect(fn).toContain('bandLine');
    expect(fn).toContain('30위 안에 붙어 본 말');
  });
});

describe('봉투가 있을 때', () => {
  /*
   * '이겨본 가장 큰 자리 경쟁 글 N개'를 내렸다(2026-09-15). 사장님 블로그에서 그 숫자가 1,690,874 였다 —
   * 넓은 말 하나를 10위로 이긴 기록이라 크기를 말해 주지 못했고, 그 숫자로 거르면 거의 다 통과했다.
   * 이제 거르는 기준인 '30위 안에 붙어 본 말'의 수와 검색량을 보여 준다.
   */
  it('30위 안에 붙어 본 말·정면·잘 이기는 이야기를 보여준다 — 경쟁 글 최대치는 안 보인다', () => {
    expect(fn).toContain('30위 안에 붙어 본 말');
    expect(fn).toContain('env.facingMax');
    expect(fn).toContain('잘 이기는 이야기');
    expect(fn).not.toContain('이겨본 가장 큰 자리');
    expect(fn).not.toContain('env.docMax');
  });
});
