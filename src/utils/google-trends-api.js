 ...
 */
async function getGoogleTrendKeywordsWithCSE() {
    try {
 ...
 */
async function getGoogleTrendKeywords() {
                ...
                for (let i = 0; i < allRows.length && result.length < 10; i++) {
                    ...
                    for (const textEl of Array.from(textElements)) {
                        ...
                        if (text &&
                            ...
                            !text.includes('탐색') &&
                            !/^[\d\s\-_]+$/.test(text)) {
                            const isDuplicate = result.some(item => item.keyword.toLowerCase() === text.toLowerCase());
                            if (!isDuplicate) {
                                result.push({
                                    keyword: text,
                                    rank: result.length + 1
