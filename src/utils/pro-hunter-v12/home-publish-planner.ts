...
const FINANCE_RE = /(계좌|환급|공제|금리|이자|청약|적금|예금|대출|보험|카드|수수료|혜택|조건|재테크|투자)/;
const LIVING_RE = /(장마|제습|습기|냄새|빨래|청소|곰팡이|원룸|집안|생활비|정리|관리|수납|가전|전기세|소음)/;
...
type HomeIntent = 'policy' | 'commerce' | 'travel' | 'entertainment' | 'health' | 'finance' | 'living' | 'general';
...
function detectCategoryIntent(category?: string): HomeIntent | null {
    ...
    if (['finance', 'business', 'career', 'sidejob', 'realestate'].includes(c) || /(재테크|투자|부업|n잡|부동산|계좌|환급|공제)/i.test(raw)) {
        return 'finance';
    }
    if (['living', 'interior', 'home', 'homelife'].includes(c) || /(생활|인테리어|집안|장마|제습|청소|원룸)/.test(raw)) {
        return 'living';
    }
    ...
    if (['parenting', 'pregnancy', 'education', 'wedding', 'issue', 'general'].includes(c)) {
        return 'general';
    }
    return null;
}
...
function detectIntent(keyword: string, category?: string): HomeIntent {
    ...
    if (LIVING_RE.test(text)) return 'living';
...
const HOME_TITLE_ACTION_RULES: Record<HomeIntent, RegExp[]> = {
    ...
    finance: [
        /조건|혜택|금액|금리|만기|납입|환급|공제|수수료|비용|비교|주의|변경/,
        /직장인|청년|초보|가입|해지|계산|실수|체크/,
    ],
    living: [
        /원인|해결|방법|청소|관리|점검|제습|습기|냄새|곰팡이|소음|전기세/,
        /원룸|집안|준비물|필터|세척|실수|주의|비교/,
    ],
    general: [
...
function fallbackTitles(keyword: string, intent: ReturnType<typeof detectIntent>): string[] {
    ...
    if (intent === 'finance') {
        return [
            `${kw} 조건·혜택·주의사항 한 번에 정리`,
            `${kw} 금액과 비용 비교, 가입 전 체크`,
            `${kw} 초보가 놓치기 쉬운 실수와 변경사항`,
        ];
    }
    if (intent === 'living') {
        return [
            `${kw} 원인과 해결 방법, 오늘 바로 점검할 것`,
            `${kw} 청소·관리 체크리스트와 흔한 실수`,
            `${kw} 비용·전기세·주의사항까지 비교 정리`,
        ];
    }
    return [
...
function titleReason(title: TitleCtrResult, intent: ReturnType<typeof detectIntent>, risky: boolean): string {
    ...
    if (intent === 'finance') return '조건·비용 판단 의도 대응';
    if (intent === 'living') return '생활 문제 해결 의도 대응';
...
function hookFor(intent: ReturnType<typeof detectIntent>): string {
    ...
    if (intent === 'finance') return '조건·금액·주의사항을 먼저 정리하고, 가입/해지 판단 포인트로 확장';
    if (intent === 'living') return '원인과 해결 방법을 먼저 제시하고, 집에서 바로 점검할 체크리스트로 연결';
...
function outlineFor(keyword: string, intent: ReturnType<typeof detectIntent>): string[] {
    ...
    if (intent === 'finance') return ['핵심 요약', '가입 조건', '금액/혜택', '비용과 수수료', '주의사항'];
    if (intent === 'living') return ['핵심 요약', '원인', '해결 방법', '청소/관리 체크리스트', '주의할 실수'];
...
function mustIncludeFor(keyword: string, intent: ReturnType<typeof detectIntent>): string[] {
    ...
    if (intent === 'finance') return ['가입 조건', '금액/혜택', '비용/수수료', '주의사항'];
    if (intent === 'living') return ['원인', '해결 방법', '청소/관리 방법', '주의사항'];
...
function avoidFor(intent: ReturnType<typeof detectIntent>): string[] {
    ...
    if (intent === 'finance') return [...base, '수익 보장 표현'];
    if (intent === 'living') return [...base, '검증 없는 만능 해결법'];
    return base;
}
