...
export function buildBriefPrompt(field: string, facts: FactCard[], today: Date, maxBriefs = 3, exclude: ReadonlyArray<string> = []): string {
  ...
  return [
    ...
    '각 글감은 JSON 객체다:',
    '{"title": "대표 제목 — 위 유튜브식 공식(검색어+강한 훅+사람 냄새)을 그대로 지킨다. 30자 안팎. 기사 제목·보도 말투 금지",',
    ...
    '제목 규칙(네이버 블로그)',
    '',
    '  ■ 먼저 이것부터 — **유튜브 제목을 짓듯이 지어라**(사장님 2026-09-11).',
    '    기사 제목을 옮기지 마라. 그건 누구나 뽑을 수 있고 아무도 안 누른다.',
    '    앵커(사장님이 직접 쓴 기준 문장, 복붙 금지 · 결만 배워라):',
    '      "포켓몬고 위치정보 오류 이러니까 바로 풀리네요"',
    '      "김부장 드라마만 보고 결말 안다고 하면 큰일 납니다"',
    '    공식: 검색어 + 자극적인 강한 훅 + 사람 냄새 — 셋 다 있어야 한다.',
    '    · 자극은 **지어낸 감탄사가 아니라 진짜 손해·억울함·불안**에서 온다(헛돈·놓침·큰일).',
    '      사실을 지어내지 말고 감정의 진폭만 키워라.',
    '    · 답의 **존재만** 알리고 내용은 숨겨라. "이러니까"의 정체는 본문에 있다.',
    '      무엇이 아닌지는 말해도 되고, 무엇인지를 적으면 실패다.',
    '    · 말하듯 끝내라 — ~네요 · ~더라고요 · ~합니다 · ~해요. 보도 말투 금지:',
    '      "나선다 · 밝혔다 · 전망이다 · 꼽힌 이유 · 원서접수 · 개최 · 합류했다" 로 끝나면 탈락이다.',
    '    · 자기검문 한 번: "AI가 지은 티가 나는가? 누구나 다 짓는 제목인가?" 하나라도 예면 다시 지어라.',
    '',
    '  ■ 그 다음에 세 군데를 노린다. target 마다 노리는 곳이 다르니 규칙도 다르다.',
    '    셋 다 위 공식을 지킨 채로 만든다 — 기계에 맞추느라 사람이 안 누를 제목을 만들지 마라.',
    '',
    '  [target: "검색"] 네이버 검색 결과에 걸리는 제목',
    '    · keywords 중 하나로 **문장을 시작**하라. 그 말을 토씨 하나 바꾸지 말고 그대로 앞에 놓아라.',
    '      (틀림: "왜 반도체 계약학과로 지원이 쏠렸을까" ← 검색어가 앞에 없다)',
    '    · 그 뒤에 **왜 눌러야 하는지**를 붙여라. 무엇을 좁히는지만 적으면 기사 제목이 된다.',
    '      (틀림: "독감 유행 왜 예년보다 한 달 빨라졌나" ← 신문이 쓰는 꼴)',
    '      (맞음: "독감 유행 한 달 빨라진 이유 병원에서 듣고 놀랐어요")',
    '    · 30자 안팎.',
    '',
    '  [target: "AI답변"] 네이버 AI 브리핑·스마트블록이 답으로 물어 가는 제목',
    '    · 여기도 keywords 중 하나로 **문장을 시작**한다. 그 뒤에 사람들이 실제로 궁금해하는 것을 붙인다.',
    '    · 답을 요구하는 꼴로 끝내되 **말하듯** 끝내라.',
    '      (틀림: "곽빈 올 시즌 성적 어떻게 되는지" ← 검색창에 치는 말이지 제목이 아니다)',
    '      (맞음: "하반기 공채 지원 자격 보고 접었다가 다시 열었어요")',
    '    · 사람이 검색창에 실제로 치는 말로 시작하되, 문장은 사람이 말하듯 끝난다. 수사의문문 금지.',
    '',
    '  [target: "인용"] 생성형 AI 가 근거로 인용하기 좋은 제목',
    '    · 여기도 keywords 중 하나로 **문장을 시작**한다.',
    '    · 카드의 숫자·날짜를 **미끼로만** 쓴다 — 답이 아니라 궁금하게 만드는 자리에.',
    '      (틀림: "곽빈 올 시즌 11승 7패 평균자책점 2.26 기록" ← 답을 다 줬다)',
    '      (맞음: "곽빈 올 시즌 성적 보고 와일드카드 논란이 왜 났는지 알겠더라고요")',
    '    · 숫자를 넣되 그 숫자가 **무엇을 뜻하는지는 본문에** 둔다.',
    '',
 ...
 */
/**
 * 보도 말투 꼬리 — 기사 제목을 그대로 옮겨 놓은 것을 잡는다.
 *
 * 실측(2026-09-11 오후 회차): 대표 제목 여덟 개가 전부 통신사 헤드라인이었다.
 *   "곽빈 아시안게임 키플레이어로 꼽힌 이유" · "LG전자 … 연내 양산 나선다"
 *   "하반기 공채 삼성·한화손보 9월 중순 원서접수" · "상병수당 2027년 도입 앞두고 열린 공청회"
 * 사장님: "누구나 생각할수있고 ai로 돌리면 누구나 뽑을수있는 그런 제목이야 나같아도 안쓸것같은데".
 *
 * 여기 걸리는 말은 **신문이 쓰는 꼬리**다. 블로그 제목은 사람이 말하듯 끝난다.
 * 교리(feedback_home_title_doctrine)의 앵커 "포켓몬고 위치정보 오류 이러니까 바로 풀리네요" 는
 * 이 목록에 하나도 안 걸린다 — 걸리면 규칙이 틀린 것이다.
 */
export const NEWSY_TAILS: readonly string[] = [
  '나선다', '밝혔다', '전망이다', '예정이다', '계획이다', '방침이다', '분석된다', '보인다',
  '꼽힌 이유', '꼽혔다', '열린 공청회', '개최', '원서접수', '접수 시작', '돌입', '착수',
  '한다는 분석', '기록', '확정', '발표', '공개', '출시', '합류했다', '선정', '추진',
];

/** 사람이 말하듯 끝나는가. 교리 3항(구어체) — ~네요/~어요/~더라고요/~합니다. */
export function hasHumanVoice(text: string): boolean {
  // 니다 는 '습니다' 와 'ㅂ니다'(납니다·갑니다·됩니다)를 같이 덮는다 — 교리 앵커가 둘 다 쓴다.
  return /(네요|어요|아요|더라고요|던데요|거든요|니다|해요|겠어요|잖아요|드려요|봤어요|했어요)\s*[.!?]?\s*$/.test(String(text || '').trim());
}

/**
 * 기사 제목인가.
 *
 * 보도 꼬리로 끝나면 기사다. 사람 말투로 끝나면 기사가 아니다 —
 * 사람 말투가 우선이다("…라고 밝혔더라고요" 같은 것을 기사로 몰면 안 된다).
 */
export function isNewsyTitle(text: string): boolean {
  const t = String(text || '').trim();
  if (!t) return false;
  if (hasHumanVoice(t)) return false;
  return NEWSY_TAILS.some((tail) => t.endsWith(tail));
}
...
export function sanitizeTitles(raw: unknown, mainTitle: string, limit = 4, keywords: readonly string[] = []): BriefTitle[] {
  ...
  for (const item of raw) {
    ...
    // 기사 제목을 그대로 옮긴 것은 버린다 — 누구나 뽑을 수 있고 아무도 안 누른다(사장님 2026-09-11).
    if (isNewsyTitle(text)) continue;
   ...
   * 갈래 안에서는 **사람 말투를 앞세운다.**
   *
   * 실측(2026-09-11 오후 73개): 기사형 꼬리에 걸린 건 7개뿐인데 사람 말투는 **0개**였다.
   * 꼬리 목록으로는 "독감 유행 왜 예년보다 한 달 빨라졌나" 같은 신문 제목을 못 잡는다.
   * 그렇다고 사람 말투 없는 것을 다 버리면 그 회차가 통째로 빈다 — 버리지 않고 순서만 바꾼다.
   */
  const pools = order.map((t) => kept.filter((k) => k.target === t)
    .sort((a, b) => Number(hasHumanVoice(b.text)) - Number(hasHumanVoice(a.text))));
