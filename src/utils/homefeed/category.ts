...
const CATEGORY_RULES: ReadonlyArray<{ category: HomefeedCategory; re: RegExp }> = [
  ...
  /**
   * 2026-09-17 보강: 스타 이름 + 사건만 있는 제목이 '미분류'로 빠졌다(실측 10건 중 7건).
   * 연예 기사는 직업 · 소속 · 활동 맥락을 거의 항상 달고 나온다 — 그 말을 함께 본다.
   */
  { category: 'entertainment', re: /(배우|가수|아이돌|걸그룹|보이그룹|드라마|예능|방송|열애|결혼|이혼|컴백|유튜버|영화|시청률|출연|\bMC\b|\bBJ\b|연예|소속사|앨범|뮤직비디오|팬미팅|콘서트|래퍼|개그맨|코미디언|트로트|아나운서|리메이크|무대|공연장|소극장|시상식|화보|예능인|전처|전남편|전부인|행사비|대세|근황|샤라웃|데뷔|은퇴설|열애설)/giu },
  /**
   * 2026-09-17 보강: 선수 이름과 경기 서사만 있는 제목이 통째로 '미분류'로 빠졌다(실측 10건 중 8건).
   * 미분류는 실물 사진 전략(visual.ts REAL_FIRST_CATEGORIES)도 못 받아 카드가 약해진다.
   * 구단 · 리그 약칭과 경기 결과 말을 함께 본다.
   */
  { category: 'sports', re: /(야구|축구|농구|배구|골프|선수|감독|경기(?!\s*(?:도|남부|북부|권|장|침체|둔화|회복|불황|부양|지표|악화|호전))|우승|준우승|리그|올림픽|월드컵|국가대표|홈런|선발\s?제외|명단\s?제외|압승|완승|쾌승|역전승|참패|완패|연승|연패|강등|승격|이적|영입|맞대결|해트트릭|멀티골|완봉|세이브|타율|승점|코리아컵|\bKBO\b|\bEPL\b|\bMLB\b|\bML\b|\bSON\b|토트넘|전북현대|키움|\bNC\b)/giu },
