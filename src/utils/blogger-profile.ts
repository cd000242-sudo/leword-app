...
/**
 * 키워드가 사용자 카테고리에 속하는지 점수 산출
 * +30: 강한 매칭 (사용자 카테고리 패턴 직접 매칭)
 * +0: 중립 (매칭 없음)
 * -15: 사용자 ban 카테고리 매칭 (선택 안 한 finance/health/it 같은 전문 영역)
 */
export function calculateProfileAffinity(keyword: string, profile: BloggerProfile | null): number {
  ...
  // 내 카테고리 아닌데 다른 전문 영역 매칭이면 페널티
  // (단, 일반 commercial / longtail 은 영향 없음)
