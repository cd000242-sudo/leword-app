 ...
 *
 * [정본 결정] SSS = classic(고볼륨 저경쟁) OR winnable(저볼륨 문서수≪검색량).
 *   2026-07-15 에 winnable 을 껐다가 2026-09-11 사장님 지시로 되살렸다 — CLAUDE.md 규격이
 *   내내 winnable 을 SSS 로 적고 있었고, 끈 동안 초보자가 이길 만한 작은 말이 전부 탈락했다.
 ...
 *
 * 되살린 경위(2026-09-11, 사장님 "되살려"): 2026-07-15 커밋 dcfd6ffd 가 이 길을 껐는데
 * CLAUDE.md 의 등급 시스템은 계속 이 길을 규격으로 적고 있었다. 규격과 코드가 두 달 갈라진 채였고,
 * 그 사이 golden-floor-winnability-rank 테스트가 계속 깨져 있었다 —
 * 실측(2026-09-11)에서 주휴수당 신청방법(900/120)·치아보험 면책기간 조회(700/90)가 통째로 탈락하고
 * 검색량 35,000짜리 대형 헤드만 남았다. 초보자가 실제로 이기는 건 작은 말이다.
 * 임계값은 지어내지 않고 껐던 커밋에서 그대로 되살렸다.
 */
