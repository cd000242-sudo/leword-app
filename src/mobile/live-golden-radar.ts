...
const LIVE_CACHE_PROMOTION_BATCH_SIZE = 8;
const LIVE_CACHE_PROMOTION_BATCH_TIMEOUT_MS = 18_000;
...
function isMismatchedLiveEventIntent(keyword: string): boolean {
  ...
  const eventLike = holidayLike || LOW_VALUE_EVENT_TOPIC_RE.test(clean) || NON_PRODUCT_NEWS_BASE_RE.test(clean);
...
export class MobileLiveGoldenRadar {
  ...
  private queueMeasuredProbeCandidates(
    ...
    for (const raw of candidates) {
      ...
      if (!isSearchAdMeasurableLiveCandidate(clean, inferred || categoryId, now)) continue;
  ...
  private loadMeasuredProbeQueueFromFile(): void {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        if (!isSearchAdMeasurableLiveCandidate(keyword, category || 'all', now)) continue;
        seen.add(compact);
        this.pendingMeasuredProbeQueue.push({
          keyword,
  ...
  private saveMeasuredProbeQueueToFile(): void {
    ...
    try {
      ...
      const payload = {
        ...
        items: this.pendingMeasuredProbeQueue.slice(0, LIVE_PROBE_QUEUE_MAX_ITEMS),
      };
      ...
      .map((keyword, index) => ({
        ...
        score: preVolumeCandidateScore(keyword, categoryId)
          ...
          + (queuedProbeCandidateIds.has(keywordCompactId(keyword))
            ? 520 + Math.min(260, Math.max(0, queuedProbePriorityById.get(keywordCompactId(keyword)) || 0))
            : 0),
      }))
