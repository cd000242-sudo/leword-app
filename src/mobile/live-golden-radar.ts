...
const PHONE_PRODUCT_BASE_RE = /(?:\uC544\uC774\uD3F0|\uAC24\uB7ED\uC2DC|\uD734\uB300\uD3F0|\uC2A4\uB9C8\uD2B8\uD3F0|\biPhone\b|\bGalaxy\b)/iu;
const PHONE_HOUSING_CONTEXT_MISMATCH_RE = /(?:(?:\uC544\uC774\uD3F0|\uAC24\uB7ED\uC2DC|\uD734\uB300\uD3F0|\uC2A4\uB9C8\uD2B8\uD3F0|\biPhone\b|\bGalaxy\b).{0,14}(?:\uC6D0\uB8F8|\uC790\uCDE8\uBC29|1\uC778\uAC00\uAD6C|\uC18C\uD615|\uC800\uC18C\uC74C|\uC18C\uC74C|\uC804\uAE30\uC694\uAE08|\uC804\uAE30\uC138|\uC124\uCE58\uBE44|\uD544\uD130|\uAD50\uCCB4\uC8FC\uAE30).{0,14}(?:\uAD6C\uB9E4\uCC98|\uCD5C\uC800\uAC00|\uAC00\uACA9|\uCD94\uCC9C|\uD6C4\uAE30|\uBE44\uAD50|\uD560\uC778|\uCFE0\uD3F0|\uCD9C\uC2DC\uC77C|\uBC1C\uB9E4\uC77C|\uC2A4\uD399)|(?:\uC6D0\uB8F8|\uC790\uCDE8\uBC29|1\uC778\uAC00\uAD6C|\uC18C\uD615|\uC800\uC18C\uC74C|\uC18C\uC74C|\uC804\uAE30\uC694\uAE08|\uC804\uAE30\uC138|\uC124\uCE58\uBE44|\uD544\uD130|\uAD50\uCCB4\uC8FC\uAE30).{0,14}(?:\uC544\uC774\uD3F0|\uAC24\uB7ED\uC2DC|\uD734\uB300\uD3F0|\uC2A4\uB9C8\uD2B8\uD3F0|\biPhone\b|\bGalaxy\b).{0,14}(?:\uAD6C\uB9E4\uCC98|\uCD5C\uC800\uAC00|\uAC00\uACA9|\uCD94\uCC9C|\uD6C4\uAE30|\uBE44\uAD50|\uD560\uC778|\uCFE0\uD3F0|\uCD9C\uC2DC\uC77C|\uBC1C\uB9E4\uC77C|\uC2A4\uD399))/iu;
...
function selectMeasuredPublishableFallbackItems<T extends MobileLiveGoldenBoardItem>(
  ...
  for (const item of sorted) {
    ...
    if (
      ...
      && isFinalVisibleLiveBoardMetric(item, now)
...
function isFinalVisibleLiveBoardMetric(
  ...
  const measuredBoardReference = isMeasuredBoardReferenceMetric(metric, now);
  const rawGrade = String(metric.grade || '').toUpperCase().replace(/[^A-Z]/g, '');
  const finalGrade = rawGrade === 'C' ? 'C' : normalizeGrade(metric.grade, finiteNumber(metric.score) || 0);
  if ((GRADE_RANK[finalGrade] || 0) < LIVE_BOARD_MIN_VISIBLE_GRADE_RANK) return false;
  if (
    ...
    || isStandaloneToolHeadKeyword(keyword)
  ...
  if (ratio < 0.5 && !exactMeasuredDisplayFallback && !measuredBoardReference) return false;
...
const LIVE_FINAL_OPAQUE_POLICY_LABEL_RE = /(?:\uC0AC\uD68C[^\uAC00-\uD7A3]{0,3}\uBB38\uD654[^\uAC00-\uD7A3]{0,3}\uCCAD\uB144.{0,8}\uC2E0\uCCAD\s*\uBC29\uBC95|\uC815\uB840\uB300\uD654.{0,8}(?:\uC2E0\uCCAD|\uC9C0\uAE09\uC77C|\uC0AC\uC6A9\uCC98|\uAE08\uC561)|(?:\uC0DD|\uC2E4|\uBD80|\uCD1D)?\uB300\uC0C1\uC73C\uB85C.{0,8}(?:\uC2DC\uD589|\uB9C8\uAC10|\uC9C0\uAE09\uC77C|\uC2E0\uCCAD|\uC870\uAC74))/u;
const LIVE_SEMANTIC_DEVICE_TOKENS = ['\uC544\uC774\uD3F0', '\uAC24\uB7ED\uC2DC', '\uB178\uD2B8\uBD81', '\uBAA8\uB2C8\uD130'] as const;
const LIVE_SEMANTIC_HOME_TOKENS = ['\uC790\uCDE8\uBC29', '\uC6D0\uB8F8', '1\uC778\uAC00\uAD6C'] as const;
const LIVE_SEMANTIC_COMMERCE_TOKENS = ['\uAD6C\uB9E4\uCC98', '\uCD5C\uC800\uAC00', '\uD560\uC778', '\uCFE0\uD3F0', '\uC124\uCE58\uBE44'] as const;
const LIVE_SEMANTIC_UTILITY_TOKENS = ['\uC804\uAE30\uC694\uAE08', '\uC804\uAE30\uC138', '\uC18C\uC74C'] as const;
const LIVE_SEMANTIC_COMPARE_TOKENS = ['\uBE44\uAD50', '\uC870\uD68C', '\uACC4\uC0B0', '\uBE44\uC6A9'] as const;
const LIVE_SEMANTIC_HOME_APPLIANCE_TOKENS = ['\uB85C\uBD07\uCCAD\uC18C\uAE30', '\uBB34\uC120\uCCAD\uC18C\uAE30', '\uCC28\uB7C9\uC6A9\uCCAD\uC18C\uAE30', '\uCCAD\uC18C\uAE30', '\uC81C\uC2B5\uAE30', '\uACF5\uAE30\uCCAD\uC815\uAE30', '\uC5D0\uC5B4\uCEE8'] as const;
const LIVE_SEMANTIC_RELEASE_TOKENS = ['\uCD9C\uC2DC\uC77C', '\uBC1C\uB9E4\uC77C', '\uC0AC\uC804\uC608\uC57D', '\uACF5\uC2DD\uC601\uC0C1'] as const;
const LIVE_SEMANTIC_SUPPLEMENT_TOKENS = ['\uB8E8\uD14C\uC778', '\uC624\uBA54\uAC003', '\uBE44\uD0C0\uBBFC', '\uC720\uC0B0\uADE0', '\uCF5C\uB77C\uAC90'] as const;
const LIVE_SEMANTIC_MEDICAL_ADMIN_TOKENS = ['\uAC80\uC0AC\uBE44\uC6A9', '\uBCF4\uD5D8\uC801\uC6A9', '\uC2E4\uBE44\uCCAD\uAD6C'] as const;
const LIVE_SEMANTIC_PERSON_OR_GROUP_TOKENS = ['\uC815\uCCAD\uB798', '\uC758\uCD1D', '\uCC38\uC11D', '\uB300\uD45C', '\uD6C4\uBCF4', '\uAC10\uB3C5', '\uC120\uC218'] as const;
const LIVE_SEMANTIC_POLICY_TAIL_TOKENS = ['\uC9C0\uAE09\uC77C', '\uC2E0\uCCAD', '\uB300\uC0C1', '\uC790\uACA9', '\uC0AC\uC6A9\uCC98', '\uAE08\uC561'] as const;

function hasAnyLiveSemanticToken(value: string, tokens: readonly string[]): boolean {
  return tokens.some((token) => value.includes(token));
}

function isLiveSemanticCollisionProbe(keyword: string): boolean {
  const compact = normalizeKeyword(keyword).replace(/\s+/g, '');
  if (!compact) return false;
  const hasDevice = hasAnyLiveSemanticToken(compact, LIVE_SEMANTIC_DEVICE_TOKENS);
  const hasHome = hasAnyLiveSemanticToken(compact, LIVE_SEMANTIC_HOME_TOKENS);
  const hasCommerce = hasAnyLiveSemanticToken(compact, LIVE_SEMANTIC_COMMERCE_TOKENS);
  const hasUtility = hasAnyLiveSemanticToken(compact, LIVE_SEMANTIC_UTILITY_TOKENS);
  const hasCompare = hasAnyLiveSemanticToken(compact, LIVE_SEMANTIC_COMPARE_TOKENS);
  const hasAppliance = hasAnyLiveSemanticToken(compact, LIVE_SEMANTIC_HOME_APPLIANCE_TOKENS);
  const hasRelease = hasAnyLiveSemanticToken(compact, LIVE_SEMANTIC_RELEASE_TOKENS);

  if (hasDevice && hasHome && (hasCommerce || hasUtility)) return true;
  if (hasDevice && hasUtility && hasCompare) return true;
  if (hasAppliance && hasHome && hasCommerce) return true;
  if (hasAppliance && hasRelease && (hasHome || hasCommerce || hasCompare)) return true;
  if (hasAnyLiveSemanticToken(compact, LIVE_SEMANTIC_SUPPLEMENT_TOKENS) && hasAnyLiveSemanticToken(compact, LIVE_SEMANTIC_MEDICAL_ADMIN_TOKENS)) return true;
  if (hasAnyLiveSemanticToken(compact, LIVE_SEMANTIC_PERSON_OR_GROUP_TOKENS) && hasAnyLiveSemanticToken(compact, LIVE_SEMANTIC_POLICY_TAIL_TOKENS)) return true;
  if (compact.includes('\uC1A1\uC9C0\uD638\uBC14\uB2E4\uD558\uB298\uAE38') && compact.includes('\uC7A5\uB2E8\uC810') && (compact.includes('\uC608\uC57D') || compact.includes('\uC8FC\uCC28'))) return true;
  if (hasAnyLiveSemanticToken(compact, ['\uC790\uCDE8\uBC29', '\uC6D0\uB8F8', '\uC18C\uC74C', '\uC804\uAE30\uC694\uAE08', '\uB80C\uD0C8', '\uCD5C\uC800\uAC00', '\uAD6C\uB9E4\uCC98', '\uB178\uD2B8\uBD81', '\uB85C\uBD07\uCCAD\uC18C\uAE30']) && compact.includes('\uB808\uC2DC\uD53C')) return true;
  return false;
}
...
function isTrustedPolicyAudienceWriterReadyKeyword(keyword: string, categoryId = ''): boolean {
  const clean = normalizeKeyword(keyword);
  if (!clean) return false;
  if (LIVE_FINAL_OPAQUE_POLICY_LABEL_RE.test(clean) || LIVE_FINAL_PRODUCT_CONTEXT_NOISE_RE.test(clean) || isLiveSemanticCollisionProbe(clean)) return false;
  const inferred = inferLiveCategory(clean, categoryId || 'all');
  const policyLike = inferred === 'policy'
    || LIVE_POLICY_SIGNAL_RE.test(clean)
    || SEARCHAD_POLICY_PRODUCT_BASE_RE.test(clean)
...
function isSyntheticNoEffectLiveProbe(keyword: string): boolean {
  ...
  return isSemanticallyMismatchedMeasuredProbe(clean)
    ...
    || PHONE_HOUSING_CONTEXT_MISMATCH_RE.test(clean)
    ...
    || isLiveSemanticCollisionProbe(clean)
    || (
      PRODUCT_BASE_SIGNAL_RE.test(clean)
      && (
...
function measuredProbeDetailModifiersForBase(base: string, categoryId: string): string[] {
  ...
  const modifiers = LIVE_MEASURED_PROBE_DETAIL_MODIFIERS[inferred]
    ...
    || [];
  return modifiers
    .filter((modifier) => !PHONE_PRODUCT_BASE_RE.test(clean) || !HOUSING_REAL_ESTATE_CONTEXT_RE.test(modifier))
    .filter((modifier) => !PHONE_PRODUCT_BASE_RE.test(clean) || !/(?:1\uC778\uAC00\uAD6C|\uC18C\uD615|\uC800\uC18C\uC74C|\uC18C\uC74C|\uC804\uAE30\uC694\uAE08|\uC804\uAE30\uC138|\uC124\uCE58\uBE44|\uD544\uD130|\uAD50\uCCB4\uC8FC\uAE30)/u.test(modifier))
    .filter((modifier) => !keywordAlreadyHasIntent(clean, modifier))
    .slice(0, 5);
}
...
function isMeasuredBoardReferenceMetric(
  ...
  if (!keyword) return false;
  const metric = { ...item, keyword } as MobileLiveGoldenBoardItem;
  const rawGrade = String(item.grade || '').toUpperCase().replace(/[^A-Z]/g, '');
  const grade = rawGrade === 'C' ? 'C' : normalizeGrade(item.grade, finiteNumber(item.score) || 0);
  if ((GRADE_RANK[grade] || 0) < LIVE_BOARD_MIN_VISIBLE_GRADE_RANK) return false;
  if (!hasCompleteLiveGoldenMetrics(item)) return false;
  if (item.isSearchVolumeEstimated || item.isDocumentCountEstimated) return false;
  if (isMalformedLiveKeyword(keyword) || isStaleOrFutureLiveKeyword(keyword, now)) return false;
  if (isInvalidNonProductCommerceExpansion(keyword)) return false;
  if (isThinProfileIntentKeyword(keyword) || isNoisyLiveSeed(keyword) || isOverExpandedLiveCandidate(keyword)) return false;
  if (isSyntheticNoEffectLiveProbe(keyword) || isWeakAutogeneratedProbeCombo(keyword)) return false;
  if (isSemanticallyMismatchedMeasuredProbe(keyword)) return false;
  if ((!isActionableSportsLiveEventKeyword(keyword) && isUltimateLowValueLookupKeyword(keyword)) || isLowValueLiveCandidate(keyword)) return false;
  if (isLottoLookupKeyword(keyword) || isLowAdsenseLookupKeyword(keyword) || isBrandSafetyNewsKeyword(keyword)) return false;
  if (!isBlogActionableBoardMetric(metric) && !isStandaloneToolHeadKeyword(keyword)) return false;
  const judged = applyKeywordAiJudge(metric, { now, downgradeExcluded: false });
  const ai = judged.aiJudge;
  if (!ai || ai.verdict === 'exclude' || ai.spamRisk === 'high') return false;
  if (ai.score < 70 && !hasAdsenseNeedIntent(keyword)) return false;
  if (ai.needIntent === 'weak' && !hasAdsenseNeedIntent(keyword)) return false;
  return true;
}

function isPersistableMeasuredBoardReferenceMetric(
  item: Partial<MobileKeywordMetric> & { keyword?: string },
  now: Date = new Date(),
): boolean {
  const keyword = normalizeKeyword(item.keyword);
  if (!keyword || !isMeasuredBoardReferenceMetric(item, now)) return false;
  const metric = { ...item, keyword } as MobileLiveGoldenBoardItem;
  ...
  if (isOverbroadNoEffectBoardKeyword(metric) && !hasWriterReadySpecificity(keyword)) return false;
  return true;
}
...
export class MobileLiveGoldenRadar {
  ...
  private persistedBoardItems(): MobileLiveGoldenBoardItem[] {
    ...
    const references = this.measuredReferenceBoard()
      .filter((item) => isPersistableMeasuredBoardReferenceMetric(item, now))
      .filter((item) => {
  ...
  private mergeBoard(
    ...
    for (const keyword of keywords) {
      ...
      if (
        hasMeasuredPcMobileSplit(judgedMetric)
        && !isFinalVisibleLiveBoardMetric(judgedMetric, now)
        && !isMeasuredBoardReferenceMetric(judgedMetric, now)
      ) continue;
  ...
  private async promotePendingMeasuredCacheWithSearchAdMetrics(
    ...
    for (const row of rows) {
      ...
      if (!hasTrustedDocumentCountMeasurement(item)) continue;
      const ratio = Number((measuredVolume / docs).toFixed(2));
      const opportunityScore = liveUltimateOpportunityScore(item.keyword, measuredVolume, docs, ratio);
      const grade = normalizeLiveMetricGrade(
        item.keyword,
  ...
  private loadMeasuredResultCacheFromFile(): void {
    ...
    try {
      ...
      const pushKeyword = (row: any, fallbackCategory: string): void => {
        ...
        if (
          hasMeasuredPcMobileSplit(metric)
          && !isFinalVisibleLiveBoardMetric(metric, this.now())
          && !isMeasuredBoardReferenceMetric(metric, this.now())
        ) return;
        metrics.push(metric);
      };
  ...
  private loadBoardFromFile(replaceExisting = false): void {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const grade = totalSearchVolume !== null && documentCount !== null && documentCount > 0 && goldenRatio !== null
          ? normalizeLiveMetricGrade(keyword, row?.grade, score, totalSearchVolume, documentCount, goldenRatio)
          : normalizeGrade(row?.grade, finiteNumber(row?.score) ?? score ?? 0);
        if (grade === 'C') continue;
      ...
      const persistedCount = this.persistedBoardItems().length;
      if (rows.length > persistedCount) {
        this.saveBoardToFile();
        this.lastMessage = `loaded ${this.board.size} live golden board items; pruned ${rows.length - persistedCount} stale persisted items`;
      }
    } catch (err) {
      this.lastError = (err as Error).message || String(err);
