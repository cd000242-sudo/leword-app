...
const STRONG_NEED_RE = /(신청|대상|자격|조건|지급일|조회|예약|예매|가격|비교|추천|후기|방법|준비물|체크리스트|서류|마감|설정|사용법|주차|입장료|비용|견적|시간표|환급|지원금|혜택|청약|쿠폰|할인|구매처|최저가|가성비|사전예약|출시일|발매일|공휴일|일정|중계|계산기|정리|순위)/;
const COMMERCE_RE = /(가격|비교|추천|후기|구매|구매처|최저가|할인|쿠폰|가성비|입문|대체품|선물|렌탈|예약|예매|보험|카드|대출|연금|ETF|펀드|청약)/;
const EVERGREEN_RE = /(방법|사용법|조건|자격|서류|체크리스트|계산기|비교|추천|후기|가격|지급일|조회|신청|대상|혜택|환급|주차|입장료)/;
