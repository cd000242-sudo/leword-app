...
<body>
  ...
  <script>
    ...
    async function buildBrowserLocalApiKeywordResult(feature, seed, mode) {
      ...
      const candidates = uniqueKeywords([keyword].concat(related), targetCount);
      if (!candidates.length) throw new Error('브라우저 로컬 API 후보가 비어 있습니다.');
      updateProgress(58, '저장된 네이버 API 키로 블로그 문서수를 직접 조회합니다.');
      const docResult = await fetchBrowserDocumentCounts(candidates, Math.min(24, candidates.length)).catch(function(err) {
        errors.push(err.message || String(err));
      ...
      const sorted = sortBrowserLocalRows(rows).map(function(row, index) {
        row.rank = index + 1;
        return row;
      });
      return {
    ...
    function showTrendGraph(keyword) {
      ...
      renderLookupInsight(keyword + ' 검색량 그래프', row ? '방금 서버가 측정한 PC/모바일 검색량을 기준으로 표시합니다.' : '현재 조회 결과에 없는 키워드라 검색 바로가기를 함께 제공합니다.', [
