...
<body>
        ...
        <section class="panel main-view" id="downloads" data-view="downloads">
          ...
          <details class="settings-panel" id="naverApiSettings">
            ...
            <div class="settings-actions">
              <button class="btn primary" type="button" id="saveNaverApiSettings">저장</button>
              <button class="btn blue" type="button" id="checkNaverApiSettings">상태 확인</button>
            </div>
            <div class="settings-message" id="naverApiSettingsMessage">키 값은 저장 후 다시 표시하지 않습니다. 필요한 항목만 입력해도 기존 저장값은 유지됩니다.</div>
          </details>
        </section>
  ...
  <script>
    ...
    function statusLabel(status) {
      ...
      if (status === 'partial') return '일부 설정';
      return '미설정';
    }
    function renderNaverApiStatusMessage(payload) {
      const items = payload && payload.snapshot && Array.isArray(payload.snapshot.items) ? payload.snapshot.items : [];
      const openApi = items.find(function(item) { return item.id === 'naver-openapi'; });
      const searchAd = items.find(function(item) { return item.id === 'naver-searchad'; });
      const openStatus = openApi ? statusLabel(openApi.status) : '확인 필요';
      const adStatus = searchAd ? statusLabel(searchAd.status) : '확인 필요';
      qs('naverApiSettingsMessage').textContent = 'OpenAPI ' + openStatus + ' · SearchAd ' + adStatus + ' · 키 값은 화면에 다시 표시하지 않습니다.';
    }
    async function saveNaverApiSettings() {
      if (!requireSession()) return;
      const settings = collectNaverApiSettings();
      if (!Object.keys(settings).length) {
        qs('naverApiSettingsMessage').textContent = '저장할 네이버 API 키를 입력하세요.';
        return;
      }
      openProgress('네이버 API 키 저장', '서버 설정 저장소에 안전하게 저장하는 중입니다.');
      try {
        updateProgress(35, '입력값을 검증하고 있습니다.');
        const payload = await apiPost(endpoints.naverApiSettings, settings);
        clearNaverApiSecretInputs();
        renderNaverApiStatusMessage(payload);
        setResult({ naverApiSettings: { ok: payload.ok, savedKeys: payload.savedKeys || [] } });
        log('네이버 API 키 설정 저장 완료');
        completeProgress('네이버 API 설정이 저장되었습니다.');
      } catch (err) {
        qs('naverApiSettingsMessage').textContent = err.message;
        log('네이버 API 키 저장 실패: ' + err.message);
        failProgress(err.message);
      }
    }
    async function checkNaverApiSettings() {
      if (!requireSession()) return;
      openProgress('네이버 API 상태 확인', '저장된 키 상태를 확인하고 있습니다.');
      try {
        const payload = await apiGet(endpoints.apiStatus, true);
        renderNaverApiStatusMessage(payload);
        setResult({ naverApiStatus: payload.snapshot || payload });
        log('네이버 API 상태 확인 완료');
        completeProgress('네이버 API 상태 확인이 완료되었습니다.');
      } catch (err) {
        qs('naverApiSettingsMessage').textContent = err.message;
        log('네이버 API 상태 확인 실패: ' + err.message);
        failProgress(err.message);
      }
    }
