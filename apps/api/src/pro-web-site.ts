  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .public-preview-mode #goldenNotice {
      ...
      background: linear-gradient(135deg, rgba(248,194,27,.12), rgba(18,28,42,.42));
    }
    ...
    .tool-tab {
      ...
      border-radius: 8px;
      background:
        linear-gradient(180deg, rgba(21,30,44,.98), rgba(11,16,23,.98));
      color: #d9e6f7;
      padding: 14px;
      text-align: left;
      display: grid;
    ...
    .tool-tab strong { display: block; font-size: 15px; line-height: 1.35; color: #f5f8ff; word-break: keep-all; overflow-wrap: anywhere; }
    ...
    .tool-tab.active {
      ...
      background:
        linear-gradient(135deg, rgba(245,197,66,.95), rgba(183,246,92,.93));
      color: #07111f;
      box-shadow: 0 14px 30px rgba(245,197,66,.14);
    }
    .tool-tab.active strong, .tool-tab.active span, .tool-tab.active em { color: #07111f; }
    .tool-tab.active em { border-color: rgba(7,17,31,.22); background: rgba(7,17,31,.09); }
    .tool-group-tab.active, .ops-tab.active { border-color: rgba(245,197,66,.7); color: #07111f; background: linear-gradient(135deg, var(--gold), #b7f65c); }
    ...
    .auto-discovery-panel {
      ...
      border-radius: 8px;
      background:
        linear-gradient(135deg, rgba(53,211,153,.1), rgba(91,183,255,.07)),
        rgba(7,12,19,.82);
      padding: 12px;
    }
    .auto-discovery-panel strong { display: block; color: #d9fff0; font-size: 14px; margin-bottom: 4px; }
    .auto-discovery-panel span { display: block; color: #b8c8dc; font-size: 12px; line-height: 1.5; word-break: keep-all; }
    ...
    .progress-track { height: 12px; border: 1px solid var(--line); border-radius: 999px; background: #07090d; overflow: hidden; }
    ...
    .progress-message { min-height: 22px; margin-top: 12px; color: #d9e6f7; font-size: 13px; line-height: 1.5; }
    ...
    .progress-pill {
      ...
      border-radius: 8px;
      background: linear-gradient(135deg, rgba(17,25,37,.96), rgba(5,10,18,.96));
      color: #fff;
    ...
    .progress-pill i { display: block; height: 5px; margin-top: 9px; border-radius: 999px; background: #07101b; overflow: hidden; }
    ...
    .settings-check strong { display: block; font-size: 13px; color: #f5f8ff; margin-bottom: 4px; }
    ...
    .api-issue-card { border: 1px solid var(--line); border-radius: 10px; padding: 12px; background: rgba(13,24,40,.9); }
      ...
      <section class="main">
        <section class="hero main-view active" data-view="golden">
          ...
          <div class="metrics">
            ...
            <div class="metric-card"><strong id="metricBoard">0/120</strong><span>황금키워드 보드</span></div>
        ...
        <section class="panel main-view active" id="homeOutcomes" data-view="golden">
          <div class="panel-title">
            <div>
              <h2>성과 기록</h2>
              <div class="muted">황금키워드 발굴 후 실제 조회·수익·측정 글 현황을 홈에서 바로 확인합니다.</div>
            </div>
            <button class="btn blue" type="button" id="refreshHomeOutcomes">성과 새로고침</button>
          </div>
          <div class="ops-grid" id="homeOutcomeCards">
            <article class="ops-card"><h3>측정 글</h3><span class="ops-number">-</span><span class="ops-meta">Pro 로그인 후 확인</span></article>
            <article class="ops-card"><h3>전체 기록</h3><span class="ops-number">-</span><span class="ops-meta">실제 성과 기록 대기</span></article>
            <article class="ops-card"><h3>월 조회</h3><span class="ops-number">-</span><span class="ops-meta">성과 동기화 후 표시</span></article>
            <article class="ops-card"><h3>월 수익</h3><span class="ops-number">-</span><span class="ops-meta">성과 동기화 후 표시</span></article>
          </div>
        </section>
  ...
  <script>
    ...
    function setActiveView(id, options) {
      ...
      if (next === 'golden') {
        ...
        loadHomeOutcomes().catch(function(err) { log('성과 기록 갱신 실패: ' + err.message); });
      }
    ...
    async function trackTopKeywordFromResult() {
      ...
      try {
        ...
        await loadHomeOutcomes().catch(function() {});
      } catch (err) {
    ...
    function renderGoldenRows(items, exact, target) {
      ...
      qs('goldenBoardList').innerHTML = rows.map(function(item) {
        ...
        return '<article class="golden-row">'
          ...
          + '</span>' + publishDecisionInline(item) + '</div>'
    ...
    function homeOutcomeCard(title, value, meta, items) {
      const list = Array.isArray(items) && items.length
        ? '<ul class="ops-list">' + items.slice(0, 3).map(function(item) { return '<li>' + escapeHtml(item) + '</li>'; }).join('') + '</ul>'
        : '';
      return '<article class="ops-card"><h3>' + escapeHtml(title) + '</h3>'
        + '<span class="ops-number">' + escapeHtml(value) + '</span>'
        + '<span class="ops-meta">' + escapeHtml(meta) + '</span>'
        + list
        + '</article>';
    }
    function renderHomeOutcomesLocked() {
      const target = qs('homeOutcomeCards');
      if (!target) return;
      target.innerHTML = [
        homeOutcomeCard('측정 글', '-', 'Pro 로그인 후 확인', []),
        homeOutcomeCard('전체 기록', '-', '실제 성과 기록 대기', []),
        homeOutcomeCard('월 조회', '-', '성과 동기화 후 표시', []),
        homeOutcomeCard('월 수익', '-', '성과 동기화 후 표시', []),
      ].join('');
    }
    function renderHomeOutcomesError(message) {
      const target = qs('homeOutcomeCards');
      if (!target) return;
      target.innerHTML = [
        homeOutcomeCard('측정 글', '확인 필요', message || '성과 스냅샷을 불러오지 못했습니다.', []),
        homeOutcomeCard('전체 기록', '-', '잠시 후 다시 새로고침하세요.', []),
        homeOutcomeCard('월 조회', '-', '서버 응답 대기', []),
        homeOutcomeCard('월 수익', '-', '서버 응답 대기', []),
      ].join('');
    }
    function renderHomeOutcomesSnapshot(snapshot) {
      const target = qs('homeOutcomeCards');
      if (!target) return;
      const benchmark = (snapshot && snapshot.benchmark) || {};
      ...
      target.innerHTML = [
        homeOutcomeCard('측정 글', fmt(snapshot.measuredPosts || 0), '실제 성과가 연결된 글', []),
        homeOutcomeCard('전체 기록', fmt(snapshot.totalRecords || 0), '누적 성과 기록', []),
        homeOutcomeCard('월 조회', fmt(benchmark.totalMonthlyViews || 0), '측정 글 기준', []),
        homeOutcomeCard('월 수익', fmt(benchmark.totalMonthlyRevenue || 0), topKeywords.length ? '상위 성과 키워드 포함' : '측정 데이터 대기', topKeywords),
      ].join('');
    }
    async function loadHomeOutcomes() {
      if (!session || !session.accessToken) {
        renderHomeOutcomesLocked();
        return;
      }
      try {
        const payload = await apiGet(endpoints.proOutcomes, true, 6500);
        if (payload && payload.snapshot) {
          renderHomeOutcomesSnapshot(payload.snapshot);
          return;
        }
        renderHomeOutcomesError('성과 스냅샷 형식이 비어 있습니다.');
      } catch (err) {
        if (!session || !session.accessToken || isAuthErrorMessage(err.message)) {
          renderHomeOutcomesLocked();
          return;
        }
        renderHomeOutcomesError(err.message);
      }
    }
    async function loadHealth() {
      try {
        ...
        if (health.liveGolden) qs('metricBoard').textContent = (health.liveGolden.boardCount || 0) + '/' + (health.liveGolden.boardTarget || 120);
      } catch (err) {
    ...
    qs('loginForm').addEventListener('submit', async function(event) {
      ...
      try {
        ...
        await Promise.all([loadGoldenBoard(), loadHomeOutcomes(), refreshFeatureStatus().catch(function() {})]);
        loadCommerceDashboard(selectedCommercePeriod).catch(function() {});
    ...
    qs('refreshHomeOutcomes').addEventListener('click', loadHomeOutcomes);
    ...
    setInterval(loadHomeOutcomes, 60000);
  </script>
</body>
</html>`;
}
