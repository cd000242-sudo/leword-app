...
<body>
  ...
  <script>
    ...
    function renderToolFeatureResult(feature, result) {
      ...
      if (result && Array.isArray(result.keywords)) {
        ...
        const topRows = rows.slice(0, 12).map(function(row) {
          return '<li><strong>' + escapeHtml(row.keyword || '-') + '</strong><span>등급 ' + escapeHtml(row.grade || '-') + ' · 전체 ' + fmt(row.totalSearchVolume) + ' · PC ' + fmt(row.pcSearchVolume) + ' · 모바일 ' + fmt(row.mobileSearchVolume) + ' · 문서 ' + fmt(row.documentCount) + ' · 황금비 ' + fmt(row.goldenRatio) + '</span>' + publishDecisionInline(row) + keywordActionHtml(row.keyword || '') + '</li>';
        });
        const publishReady = rows.filter(function(row) {
          const decision = publishDecisionFor(row);
          return decision && decision.verdict === 'publish';
    ...
    function renderGoldenRows(items, exact, target) {
      ...
      const rows = filterFreshGoldenItems(items || []).slice(0, limit);
      if (!rows.length) {
    ...
    function renderPublicGoldenBoard(payload) {
      const primary = filterFreshGoldenItems(payload.publicPreview || []);
      const backfill = filterFreshGoldenItems(payload.clientBackfill || []);
