  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 820px) {
      .source-grid, .metrics, .workbench, .lookup-row, .golden-stats, .ops-grid, .tool-form, .tool-detail-grid, .catalog-strip, .catalog-list, .download-grid, .settings-grid { grid-template-columns: 1fr; }
  ...
  <script>
    ...
    function toolGuide(feature) {
      const guides = {
        ...
        'naver-mate': ['자동완성, 연관어, 검색량을 묶어 네이버 친화 롱테일 후보를 만듭니다.', 'PC/모바일 분리 지표를 켜면 표에서 바로 비교할 수 있습니다.', '시드 키워드를 넣을수록 더 좁고 정확하게 확장됩니다.'],
    ...
    function renderToolWaiting(feature) {
      renderToolResultPanel(feature, feature && feature.title ? feature.title : 'Pro 도구', '필요한 값만 입력하고 실행하세요. 실행 중에는 진행률 모달이 뜨고, 완료 후 이 영역에 결과만 정리됩니다.', [], []);
    }
    ...
    function renderToolFeatureResult(feature, result) {
      ...
      if (result && Array.isArray(result.keywords)) {
        ...
        renderToolResultPanel(feature, feature.title + ' 완료', '상위 후보를 먼저 표시합니다. 전체 표는 키워드 분석기 표와 이 결과 영역에서 이어서 확인할 수 있습니다.', [
