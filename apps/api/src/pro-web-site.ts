...
<body>
  <main class="shell mobile-pro-collapsed" id="proShell">
    <header class="topbar">
      ...
      <nav class="nav" id="proTopNav" aria-label="주요 메뉴">
        ...
        <a href="#settings" data-view-target="settings">환경설정</a>
  ...
  <script>
    ...
    const viewIds = ['golden', 'sources', 'lookup', 'features', 'ops', 'settings', 'downloads', 'workbench'];
    const catalogTabs = [
    ...
    function renderToolWaiting(feature) {
      renderToolResultPanel(feature, feature && feature.title ? feature.title : 'Pro 도구', '필요한 값만 입력하고 실행하세요. 진행률은 모달로 표시하고, 완료 후 이 영역에는 결과만 정리합니다.', [], []);
    }
    ...
    function formatApiError(url, status, payload) {
      ...
      if (status === 404) return '서버 API가 아직 최신 배포로 열리지 않았습니다. 서버 재시작/배포 반영 후 다시 실행하세요. (' + routePath(url) + ')';
    ...
    function isRouteMissingMessage(message) {
      return /404|endpoint not found|최신 배포|서버 재시작|route not found/i.test(String(message || ''));
    }
    ...
    async function runFeature(feature, options) {
      ...
      try {
        ...
        const statusLabel = isRouteMissingMessage(err.message) ? '서버 배포 확인' : '오류';
