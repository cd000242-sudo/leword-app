...
<body>
  ...
  <script>
    ...
    async function runUserApiServerExecutorLookup(feature, seed, mode, reason) {
      ...
      updateProgress(74, '브라우저 CORS로 직접 호출이 막혀 사용자 API 키를 서버 실행기로 전달해 조회합니다.');
    ...
    async function runFeature(feature, options, uiOptions) {
      ...
      try {
        ...
        if (shouldRunBrowserLocalFirst(feature, null) && normalizeText(displayKeyword)) {
          const localResult = await runBrowserLocalApiLookup(feature, displayKeyword, localApiLookupModeForFeature(feature, null), 'browser-local-primary');
