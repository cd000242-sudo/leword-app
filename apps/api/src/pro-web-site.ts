  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    @media (max-width: 820px) {
      .source-grid, .metrics, .workbench, .lookup-row, .golden-stats, .ops-grid, .tool-tabs, .tool-form, .tool-detail-grid, .catalog-strip, .catalog-list, .download-grid, .settings-grid { grid-template-columns: 1fr; }
...
<body>
  <main class="shell mobile-pro-collapsed" id="proShell">
    ...
    <div class="layout">
      <aside class="sidebar" id="proSidebar" aria-label="Pro 기능 탭">
        ...
        <a class="side-link" href="#features" data-view-target="features" data-feature-shortcut="youtube">유튜브 황금키워드 및 쇼츠분석</a>
      ...
      <section class="main">
        ...
        <section class="panel main-view" id="features" data-view="features">
          <div class="panel-title">
            <div>
              <h2>추가 기능</h2>
              <div class="muted">키워드 발굴 전용 기능만 서브탭으로 분리했습니다. 각 탭은 Electron 앱의 기능 모달처럼 입력, 실행 상태, 결과를 독립적으로 보여줍니다.</div>
            </div>
        ...
        <section class="panel main-view" id="settings" data-view="settings">
          ...
          <div class="settings-panel" id="naverApiSettings">
            ...
            <p class="muted" style="margin:0;">Naver OpenAPI는 문서수/연관어, SearchAd는 PC/모바일 실측 검색량, YouTube API는 유튜브 황금키워드에 사용합니다.</p>
            <div class="settings-grid">
              <div>
                <label for="naverClientId">Naver Client ID</label>
                <input class="input" id="naverClientId" autocomplete="off" />
              </div>
              <div>
                <label for="naverClientSecret">Naver Client Secret</label>
                <input class="input" id="naverClientSecret" type="password" autocomplete="off" />
              </div>
              <div>
                <label for="naverSearchAdAccessLicense">SearchAd Access License</label>
                <input class="input" id="naverSearchAdAccessLicense" autocomplete="off" />
              </div>
              <div>
                <label for="naverSearchAdSecretKey">SearchAd Secret Key</label>
                <input class="input" id="naverSearchAdSecretKey" type="password" autocomplete="off" />
              </div>
              <div>
                <label for="naverSearchAdCustomerId">SearchAd Customer ID</label>
            ...
            <div class="settings-actions">
              ...
              <a class="btn blue" href="https://developers.naver.com/apps/#/register" target="_blank" rel="noopener">Naver OpenAPI</a>
              <a class="btn blue" href="https://manage.searchad.naver.com/customers/links" target="_blank" rel="noopener">SearchAd API</a>
            </div>
  ...
  <script>
    ...
    const viewIds = ['golden', 'sources', 'lookup', 'features', 'settings', 'downloads'];
    const catalogTabs = [
    ...
    function renderToolDetail(feature) {
      ...
      target.innerHTML = '<div class="tool-detail-grid">'
        + '<div class="tool-detail-card"><strong>발굴 목표</strong><span>' + escapeHtml(guide[0]) + '</span></div>'
        + '<div class="tool-detail-card"><strong>판단 기준</strong><span>' + escapeHtml(guide[1]) + '</span></div>'
        + '<div class="tool-detail-card"><strong>결과 활용</strong><span>' + escapeHtml(guide[2]) + '</span></div>'
        + '<div class="tool-detail-card"><strong>실행 방식</strong><span>' + escapeHtml(method) + ' · 진행률 모달 표시 · 완료 후 결과만 표시</span></div>'
        + '</div>';
    }
    ...
    function renderNaverApiStatusMessage(payload) {
      ...
      qs('naverApiSettingsMessage').textContent =
        '브라우저 로컬 저장 · OpenAPI ' + apiSettingsReadyLabel(openReady)
        + ' · SearchAd ' + apiSettingsReadyLabel(adReady)
        + ' · YouTube ' + apiSettingsReadyLabel(youtubeReady)
    ...
    async function checkNaverApiSettings() {
      openProgress('API 설정 확인', '브라우저 로컬 설정을 확인하고 있습니다.');
      try {
        const settings = readUserApiSettings();
        renderNaverApiStatusMessage({ localSettings: settings });
        setResult({
        ...
        log('사용자 API 로컬 설정 확인 완료');
        completeProgress('로컬 API 설정 확인이 완료되었습니다.');
      } catch (err) {
        qs('naverApiSettingsMessage').textContent = err.message;
