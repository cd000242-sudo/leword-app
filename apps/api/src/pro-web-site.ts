...
<body>
  ...
  <script>
    ...
    async function buildBrowserLocalApiKeywordResult(feature, seed, mode) {
      ...
      let searchAdItems = [];
      try {
        searchAdItems = await fetchBrowserSearchAdKeywordTool([keyword]);
      } catch (err) {
        errors.push(err.message || String(err));
      }
      const metricMap = browserSearchAdMetricMap(searchAdItems);
      const related = searchAdItems.map(searchAdKeywordFromItem).filter(Boolean);
      const targetCount = mode === 'mindmap-expansion' ? 50 : mode === 'golden-discovery' ? 30 : 30;
      const contextKeywords = mode === 'mindmap-expansion'
        ? buildLookupContextKeywords(keyword, 120).map(function(item) { return item && item.keyword; }).filter(Boolean)
        : [];
      const candidateLimit = mode === 'mindmap-expansion' ? targetCount * 2 : targetCount;
      const candidates = uniqueKeywords([keyword].concat(related, contextKeywords), candidateLimit)
      ...
      const docLimit = mode === 'mindmap-expansion' ? Math.min(50, candidates.length) : Math.min(24, candidates.length);
    ...
    async function runBrowserLocalApiLookup(feature, seed, mode, reason) {
      ...
      openProgress('브라우저 로컬 API 실측', '서버 결과가 비어 있어 저장된 사용자 API 키로 직접 조회합니다.');
      try {
