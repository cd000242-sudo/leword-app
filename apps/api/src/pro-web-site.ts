...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .publish-decision strong { display: inline-flex; align-items: center; width: fit-content; border-radius: 999px; padding: 3px 8px; background: rgba(159,177,200,.14); color: #dbe7f7; font-size: 11px; font-weight: 1000; }
    .publish-decision.publish strong { background: rgba(53,211,153,.18); color: #9df5c7; border: 1px solid rgba(53,211,153,.36); }
    .publish-decision.conditional strong { background: rgba(14,165,233,.16); color: #075985; border: 1px solid rgba(14,165,233,.32); }
    .publish-decision.exclude strong { background: rgba(255,77,109,.14); color: #ff9aac; border: 1px solid rgba(255,77,109,.28); }
...
<body>
  ...
  <script>
    ...
    function browserLocalGrade(totalSearchVolume, documentCount) {
      ...
      if (total >= 300 && ratio >= 2) return 'S';
      if (total >= 100) return 'A';
      return 'B';
    }
    function browserLocalPublishDecision(row) {
      const grade = row && row.grade;
      const score = grade === 'SSS' ? 98 : grade === 'SS' ? 88 : grade === 'S' ? 78 : 62;
      const measured = !!(row && row.isMeasured);
      return {
        label: measured && (grade === 'SSS' || grade === 'SS') ? '발행 추천' : '검증 필요',
        verdict: measured && (grade === 'SSS' || grade === 'SS') ? 'publish' : 'conditional',
        score: measured ? score : null,
        reasons: [measured ? '사용자 API 키로 직접 측정한 검색량과 문서수 기준입니다.' : '일부 지표가 API에서 응답되지 않아 추가 검증이 필요합니다.'],
