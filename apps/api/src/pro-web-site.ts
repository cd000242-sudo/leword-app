    ...
    function renderFeatureResult(feature, result) {
      ...
      if (result && Array.isArray(result.keywords)) {
        ...
        if ((!feature || !feature.id) && activeViewId === 'lookup') {
          const rows = result.keywords || [];
          const topRows = rows.slice(0, 5).map(function(row) {
            return '<li><strong>' + escapeHtml(row.keyword || '-') + '</strong><span>전체 ' + fmt(row.totalSearchVolume) + ' · PC ' + fmt(row.pcSearchVolume) + ' · 모바일 ' + fmt(row.mobileSearchVolume) + ' · 문서 ' + fmt(row.documentCount) + ' · 등급 ' + escapeHtml(row.grade || '-') + '</span>' + keywordActionHtml(row.keyword || '') + '</li>';
          });
