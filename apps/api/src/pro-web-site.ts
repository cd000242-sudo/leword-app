  ...
  <main class="shell mobile-pro-collapsed" id="proShell">
    ...
    <div class="layout">
      ...
      <section class="main">
        ...
        <section class="panel main-view active" id="sources" data-view="golden">
          <div class="panel-title">
            <div>
              <h2>실시간 소스 보드</h2>
              <div class="muted">홈에서 네이버 / 다음 / 네이트 / 줌 / 정책 / 이슈 수집 결과를 바로 확인합니다.</div>
            </div>
  ...
  <script>
    ...
    function setActiveView(id, options) {
      ...
      if (next === 'golden') {
        ...
        loadSources().catch(function(err) { log('실시간 소스 갱신 실패: ' + err.message); });
    ...
    function renderSourceLane(lane) {
      ...
      const countEl = target.querySelector('.lane-count');
    ...
    function renderSourceError(message) {
      sourceLaneDefinitions.forEach(function(lane) {
        const target = qs('lane-' + lane.id);
        if (!target) return;
        target.innerHTML = '<h3>' + escapeHtml(lane.label) + ' <span class="lane-count">0</span></h3>'
          + '<div class="signal-list"><div class="muted">' + escapeHtml(message || '서버 응답이 지연되고 있습니다. 잠시 후 다시 새로고침하세요.') + '</div></div>';
      });
    }
    async function loadSources() {
      try {
        const url = session ? endpoints.proSources + '?limit=60' : endpoints.publicSources + '?limit=60';
        const payload = await apiGet(url, !!session, 6500);
        sourceBoardCache = payload;
        ...
        if (sourceBoardCache) {
          normalizeSourceLanes(sourceBoardCache).forEach(renderSourceLane);
          log('소스 보드 갱신 지연, 직전 결과 유지: ' + err.message);
          return;
        }
        renderSourceError(err.message);
    ...
    qs('loginForm').addEventListener('submit', async function(event) {
      ...
      try {
        ...
        await Promise.all([loadSources(), loadGoldenBoard(), loadHomeOutcomes(), refreshFeatureStatus().catch(function() {})]);
        loadCommerceDashboard(selectedCommercePeriod).catch(function() {});
